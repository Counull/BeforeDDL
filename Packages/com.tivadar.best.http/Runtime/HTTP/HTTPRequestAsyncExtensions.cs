...
namespace Best.HTTP {
    ...
    public static class HTTPRequestAsyncExtensions {
        ...
        /// </returns>
#if WITH_UNITASK
        public static UniTask<AssetBundle> GetAssetBundleAsync(this HTTPRequest request, CancellationToken token =
 default)
#else
...
#if WITH_UNITASK
        public static UniTask<HTTPResponse> GetHTTPResponseAsync(this HTTPRequest request, CancellationToken token =
 default)
#else
        ...
        /// </returns>
#if WITH_UNITASK
        public static UniTask<Texture2D> GetAsTexture2DAsync(this HTTPRequest request, CancellationToken token =
 default)
#else
...
#endif
        {
            switch (req.State) {
                ...
                case HTTPRequestStates.Finished:
                    if (!resp.IsSuccess)
                        tcs.TrySetException(CreateException(
                            $"Request finished Successfully, but the server sent an error ({resp.StatusCode} - '{resp.Message}').",
                            resp));
                    break;
