...
namespace Best.HTTP.Proxies {
    ...
    public sealed class HTTPProxy : Proxy {
                    ...
                    switch (this.Credentials.Type) {
                        case AuthenticationTypes.Basic:
                            ...
                            var token = Convert.ToBase64String(
                                Encoding.UTF8.GetBytes(this.Credentials.UserName + ":" + this.Credentials.Password));
                        ...
                        case AuthenticationTypes.Digest:
                            ...
                            if (digest != null) {
                                string authentication = digest.GenerateResponseHeader(this.Credentials, true,
                                    request.MethodType, request.CurrentUri);
        ...
        internal override void BeginConnect(ProxyConnectParameters parameters) {
            if (!this.IsTransparent || (parameters.createTunel && this.NonTransparentForHTTPS)) {
                using (var bufferedStream =
                       new WriteOnlyBufferedStream(parameters.stream, 4 * 1024, parameters.context))
                using (var outStream = new BinaryWriter(bufferedStream, Encoding.UTF8)) {
                    // https://www.rfc-editor.org/rfc/rfc9110.html#name-connect

                    string connectStr = string.Format("CONNECT {0}:{1} HTTP/1.1", parameters.uri.Host,
                        parameters.uri.Port.ToString());
                    ...
                    outStream.SendAsASCII(string.Format("Host: {0}:{1}", parameters.uri.Host,
                        parameters.uri.Port.ToString()));
                    outStream.Write(EOL);
                    ...
                    if (this.Credentials != null) {
                        switch (this.Credentials.Type) {
                            case AuthenticationTypes.Basic: {
                                // With Basic authentication we don't want to wait for a challenge, we will send the hash with the first request
                                var buff =
                                    $"Proxy-Authorization: Basic {Convert.ToBase64String(Encoding.UTF8.GetBytes(this.Credentials.UserName + ":" + this.Credentials.Password))}"
                                        .GetASCIIBytes();
                            ...
                            case AuthenticationTypes.Digest: {
                                ...
                                if (digest != null) {
                                    string authentication = digest.GenerateResponseHeader(this.Credentials, true,
                                        HTTPMethods.Connect, parameters.uri);
                                    if (!string.IsNullOrEmpty(authentication)) {
                                        ...
                                        if (HTTPManager.Logger.Level <= Loglevels.Information)
                                            HTTPManager.Logger.Information("HTTPProxy",
                                                "Sending proxy authorization header: " + auth, parameters.context);
        ...
        void OnProxyResponse(ProxyConnectParameters connectParameters, HTTPProxyResponse resp, Exception error) {
            HTTPManager.Logger.Information(nameof(HTTPProxyResponse),
                $"{nameof(OnProxyResponse)}({connectParameters}, {resp}, {error})", connectParameters.context);

            if (error != null) {
                // Resend request if the proxy response could be read && status code is 407 (authentication required) && we have credentials
                connectParameters.OnError?.Invoke(connectParameters, error,
                    resp.ReadState == HTTPProxyResponse.PeekableReadState.Finished && resp.StatusCode == 407 &&
                ...
                if (resp.StatusCode == 200) {
                    ...
                    if (!string.IsNullOrEmpty(authHeader)) {
                        ...
                        retryNeogitiation = connectParameters.AuthenticationAttempts <
                                            ProxyConnectParameters.MaxAuthenticationAttempts &&
                                            this.Credentials != null &&
                                            digest.IsUriProtected(this.Address) &&
                                            ( /*connectParameters.request == null || !connectParameters.request.HasHeader("Proxy-Authorization") ||*/
                                                digest.Stale);
                    }

                    if (!retryNeogitiation)
                        connectParameters.OnError?.Invoke(connectParameters,
                            new Exception(
                                $"Can't authenticate Proxy! AuthenticationAttempts: {connectParameters.AuthenticationAttempts} {resp}"),
                            false);
                    else {
                    ...
                    connectParameters.OnError?.Invoke(connectParameters, new Exception($"Proxy returned {resp}"),
                        false);
                }
            }
        }
