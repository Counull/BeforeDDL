...
namespace Best.HTTP.Proxies.Implementations {
    ...
    internal sealed class SOCKSV5Negotiator : IContentConsumer {
            ...
            try {
                ...
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Information("SOCKSProxy",
                        $"Sending method negotiation - buffer: {buffer.AsBuffer(count)} ", this._parameters.context);
        ...
        void SendConnect() {
            ...
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Information("SOCKSProxy",
                    $"Sending connect request - buffer: {buffer.AsBuffer(count)} ", this._parameters.context);
            ...
            try {
                switch (this._state) {
                    case NegotiationStates.MethodSelection: {
                        ...
                        if (HTTPManager.Logger.IsDiagnostic)
                            HTTPManager.Logger.Information("SOCKSProxy",
                                $"Negotiation response - count: {count} buffer: {buffer.AsBuffer(count)}",
                        ...
                        if (count != 2)
                            throw new Exception(
                                $"SOCKS Proxy - Expected read count: 2! buffer: {buffer.AsBuffer(count)}");
                            ...
                            HTTPManager.Logger.Information("SOCKSProxy",
                                "Method negotiation over. Method: " + method.ToString(), this._parameters.context);
                            switch (method) {
                                ...
                                case SOCKSMethods.UsernameAndPassword:
                                    if (this._proxy.Credentials.UserName.Length > 255)
                                        throw new Exception(
                                            $"SOCKS Proxy - Credentials.UserName too long! {this._proxy.Credentials.UserName.Length} > 255");
                                    if (this._proxy.Credentials.Password.Length > 255)
                                        throw new Exception(
                                            $"SOCKS Proxy - Credentials.Password too long! {this._proxy.Credentials.Password.Length} > 255");
                                    ...
                                    if (HTTPManager.Logger.IsDiagnostic)
                                        HTTPManager.Logger.Information("SOCKSProxy",
                                            $"Sending username and password sub-negotiation - buffer: {buffer.AsBuffer(count)} ",
                                            this._parameters.context);
                    ...
                    case NegotiationStates.ExpectAuthenticationResponse: {
                        ...
                        if (HTTPManager.Logger.IsDiagnostic)
                            HTTPManager.Logger.Information("SOCKSProxy",
                                $"Username and password sub-negotiation response - buffer: {buffer.AsBuffer(count)} ",
                                this._parameters.context);
                    ...
                    case NegotiationStates.ConnectResponse: {
                        ...
                        if (HTTPManager.Logger.IsDiagnostic)
                            HTTPManager.Logger.Information("SOCKSProxy",
                                $"Connect response - buffer: {buffer.AsBuffer(count)} ", this._parameters.context);
                        ...
                        if (count < 10)
                            throw new Exception(
                                $"SOCKS proxy: not enough data returned by the server. Expected count is at least 10 bytes, server returned {count} bytes! content: {buffer.AsBuffer(count)}");
