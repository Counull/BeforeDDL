    ...
    public sealed class EnvironmentProxyDetector : IProxyDetector {
        ...
        Proxy IProxyDetector.GetProxy(HTTPRequest request) {
            ...
            if (HTTPProtocolFactory.IsSecureProtocol(request.CurrentUri)) {
                ...
                HTTPManager.Logger.Information(nameof(EnvironmentProxyDetector),
                    $"{nameof(IProxyDetector.GetProxy)} - HTTPS_PROXY: '{proxyUrl}'", request.Context);
            }
            else {
                proxyUrl = GetEnv("HTTP_PROXY");
                HTTPManager.Logger.Information(nameof(EnvironmentProxyDetector),
                    $"{nameof(IProxyDetector.GetProxy)} - HTTP_PROXY: '{proxyUrl}'", request.Context);
            }
            ...
            if (proxyUrl == null) {
                ...
                HTTPManager.Logger.Information(nameof(EnvironmentProxyDetector),
                    $"{nameof(IProxyDetector.GetProxy)} - ALL_PROXY: '{proxyUrl}'", request.Context);
            ...
            try {
                ...
                HTTPManager.Logger.Exception(nameof(EnvironmentProxyDetector),
                    $"GetProxy - proxyUrl: '{proxyUrl}', exceptionList: '{exceptionList}'", ex, request.Context);
            }

            return null;
        }

        string GetEnv(string key) => System.Environment.GetEnvironmentVariable(key) ??
                                     System.Environment.GetEnvironmentVariable(key.ToLowerInvariant());
    }
}
#endif