...
namespace Best.HTTP.Proxies {
    internal sealed class HTTPProxyResponse : IContentConsumer {
        public PeekableReadState ReadState {
            ...
            private set {
                if (this._readState != value)
                    HTTPManager.Logger.Information(nameof(HTTPProxyResponse), $"{this._readState} => {value}",
        ...
        public void OnConnectionClosed() {
            ...
            if (this.ReadState == PeekableReadState.Content &&
                this._deliveryMode == ContentDeliveryMode.RawUnknownLength) {
                PostProcessContent();
                error = new Exception(
                    $"Proxy returned with {this.StatusCode} - '{this.Message}' : \"{this.DataAsText}\"");
            }
            else {
            ...
            switch (ReadState) {
                case PeekableReadState.StatusLine:
                    ...
                    if (statusCodeStr.Length > 0 && (byte) statusCodeStr[statusCodeStr.Length - 1] != LF &&
                        ...
                        HTTPManager.Logger.Warning(nameof(HTTPProxyResponse), "Skipping Status Message reading!",
                            this._parameters.context);
                    ...
                    if (HTTPManager.Logger.IsDiagnostic)
                        VerboseLogging(
                            $"HTTP/'{this.VersionMajor}.{this.VersionMinor}'  '{this.StatusCode}' '{this.Message}'");
                ...
                case PeekableReadState.Content:
                    switch (this._deliveryMode) {
                        case ContentDeliveryMode.Raw:
                            ProcessReadRaw(this.ContentProvider);
                            break;
                        case ContentDeliveryMode.RawUnknownLength:
                            ProcessReadRawUnknownLength(this.ContentProvider);
                            break;
                        case ContentDeliveryMode.Chunked:
                            ProcessReadChunked(this.ContentProvider);
                            break;
                    }
