...
namespace Best.HTTP.Response.Decompression {
    public sealed class BrotliDecompressor : IDecompressor {
        ...
        public (BufferSegment decompressed, bool releaseTheOld) Decompress(BufferSegment segment, bool forceDecompress,
            bool dataCanBeLarger, LoggingContext context) {
#if ((NET_STANDARD_2_1 || UNITY_2021_2_OR_NEWER) && (UNITY_EDITOR || (!UNITY_WEBGL && !(ENABLE_MONO && UNITY_ANDROID)))) && !BESTHTTP_DISABLE_BROTLI
            ...
            return (
                new BufferSegment(result, 0, dataCanBeLarger ? (int) decompressorOutputStream.Length : result.Length),
                false);
#else
            return (BufferSegment.Empty, false);
#endif
