...
namespace Best.HTTP {
    ...
    public class HTTPResponse : IDisposable {
        ...
        public bool IsSuccess {
            get {
                return (this.StatusCode >= OK && this.StatusCode < MultipleChoices) || this.StatusCode == NotModified;
            }
        }
        ...
        public bool HasHeaderWithValue(string headerName, string value) =>
            this.Headers.HasHeaderWithValue(headerName, value);
        ...
        protected void BeginReceiveContent() {
            ...
            if (!Request.DownloadSettings.DisableCache && !IsFromCache) {
                ...
                _cacheWriter = HTTPManager.LocalCache?.BeginCache(Request.MethodType, Request.CurrentUri,
                    this.StatusCode, this.Headers, this.Context);
            }
        }
        ...
        protected void CreateDownloadStream(IDownloadContentBufferAvailable bufferAvailable) {
            ...
            HTTPManager.Logger.Information(this.GetType().Name,
                $"{nameof(DownloadContentStream)} initialized with Maximum Buffer Size: {this.DownStream.MaxBuffered:N0}",
                this.Context);
            ...
            if (this.StatusCode >= OK && this.StatusCode < MultipleChoices)
                RequestEventHelper.EnqueueRequestEvent(
                    new RequestEventInfo(this.Request, RequestEvents.DownloadStarted));
        }
