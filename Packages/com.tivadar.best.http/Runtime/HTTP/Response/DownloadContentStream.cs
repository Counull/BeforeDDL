...
namespace Best.HTTP.Response {
    ...
    public class DownloadContentStream : Stream {
        ...
        public virtual bool IsCompleted {
            get => this._isCompleted && this._segments.Count == 0 && this._currentSegment == BufferSegment.Empty;
        }

        /// <summary>
        /// Gets a reference to an exception if the download completed with an error.
        /// </summary>
        public Exception CompletedWith {
            get => this._exceptionInfo?.SourceException;
        }
        ...
        /// </summary>
        public bool IsFull {
            get => this.Length >= this.MaxBuffered;
        }
        ...
        public bool IsDetached {
            ...
            set {
                if (this._isDetached != value) {
                    HTTPManager.Logger.Verbose(nameof(DownloadContentStream),
                        $"IsDetached {this._isDetached} => {value}", this.Response?.Context);
        ...
        public DownloadContentStream(HTTPResponse response, long maxBuffered,
        ...
        internal virtual void CompleteAdding(Exception error) {
            if (HTTPManager.Logger.Level <= Shared.Logger.Loglevels.Information)
                HTTPManager.Logger.Information(nameof(DownloadContentStream), $"CompleteAdding({error})",
