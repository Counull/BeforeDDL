...
namespace Best.HTTP {
    ...
    public sealed class HTTPRequest : IEnumerator {
        ...
        /// <returns>An HTTPRequest instance for the GET request.</returns>
        public static HTTPRequest CreateGet(string url, OnRequestFinishedDelegate callback) =>
        ...
        /// <param name="callback">A callback function to be called when the request is finished.</param>
        /// <returns>An HTTPRequest instance for the GET request.</returns>
        public static HTTPRequest CreateGet(Uri uri, OnRequestFinishedDelegate callback) =>
            new HTTPRequest(uri, callback);
        ...
        /// <param name="callback">A callback function to be called when the request is finished.</param>
        /// <returns>An HTTPRequest instance for the POST request.</returns>
        public static HTTPRequest CreatePost(string url, OnRequestFinishedDelegate callback) =>
            new HTTPRequest(url, HTTPMethods.Post, callback);
        ...
        /// <param name="callback">A callback function to be called when the request is finished.</param>
        /// <returns>An HTTPRequest instance for the POST request.</returns>
        public static HTTPRequest CreatePost(Uri uri, OnRequestFinishedDelegate callback) =>
            new HTTPRequest(uri, HTTPMethods.Post, callback);
        ...
        /// <param name="callback">A callback function to be called when the request is finished.</param>
        /// <returns>An HTTPRequest instance for the PUT request.</returns>
        public static HTTPRequest CreatePut(string url, OnRequestFinishedDelegate callback) =>
            new HTTPRequest(url, HTTPMethods.Put, callback);
        ...
        /// <param name="callback">A callback function to be called when the request is finished.</param>
        /// <returns>An HTTPRequest instance for the PUT request.</returns>
        public static HTTPRequest CreatePut(Uri uri, OnRequestFinishedDelegate callback) =>
            new HTTPRequest(uri, HTTPMethods.Put, callback);
        ...
        public Uri CurrentUri {
            get { return this.RedirectSettings.IsRedirected ? this.RedirectSettings.RedirectUri : Uri; }
        }

        /// <summary>
        /// A host-key that can be used to find the right host-variant for the request.
        /// </summary>
        public HostKey CurrentHostKey {
            get => HostKey.From(this);
        }
        ...
        public bool IsCancellationRequested {
            get => this.CancellationTokenSource != null ? this.CancellationTokenSource.IsCancellationRequested : true;
        }
        ...
        public HTTPRequestStates State {
            ...
            internal set {
                if (!HTTPUpdateDelegator.Instance.IsMainThread() &&
                    HTTPUpdateDelegator.Instance.CurrentThreadingMode == ThreadingMode.UnityUpdate)
                    HTTPManager.Logger.Error(nameof(HTTPRequest),
                        $"State.Set({this._state} => {value}) isn't called on the main thread({HTTPUpdateDelegator.Instance.MainThreadId})!",
                        this.Context);
                ...
                if (this._state >= HTTPRequestStates.Finished && value >= HTTPRequestStates.Finished) {
                    HTTPManager.Logger.Warning(nameof(HTTPRequest), $"State.Set({this._state} => {value})",
                        this.Context);
                    return;
                }

                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(HTTPRequest), $"State.Set({this._state} => {value})",
                        this.Context);
        ...
        public void EnumerateHeaders(OnHeaderEnumerationDelegate callback, bool callBeforeSendCallback) {
            ...
            if (!HasHeader("Connection"))
                AddHeader("Connection",
                    hostSettings.HTTP1ConnectionSettings.TryToReuseConnections ? "Keep-Alive, TE" : "Close, TE");
            ...
            if (hostSettings.HTTP1ConnectionSettings.TryToReuseConnections /*&& !HasHeader("Keep-Alive")*/) {
                ...
                int seconds =
                    (int) Math.Ceiling(hostSettings.HTTP1ConnectionSettings.MaxConnectionIdleTime.TotalSeconds + 1);
        ...
        public void Abort() {
            ...
            if (this.TimeoutSettings.IsTimedOut(HTTPManager.CurrentFrameDateTime)) {
                RequestEventHelper.EnqueueRequestEvent(new RequestEventInfo(this,
                    this.TimeoutSettings.IsConnectTimedOut(HTTPManager.CurrentFrameDateTime)
                        ? HTTPRequestStates.ConnectionTimedOut
                        : HTTPRequestStates.TimedOut, null));
            }
            else
