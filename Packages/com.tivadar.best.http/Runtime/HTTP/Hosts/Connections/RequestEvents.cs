...
namespace Best.HTTP.Hosts.Connections {
        ...
        public override string ToString() {
            switch (this.Event) {
                case RequestEvents.Upgraded:
                    return string.Format("[RequestEventInfo Event: Upgraded, Source: {0}]",
                        this.SourceRequest.Context.Hash);
                case RequestEvents.DownloadProgress:
                    return string.Format(
                        "[RequestEventInfo Event: DownloadProgress, Progress: {1}, ProgressLength: {2}, Source: {0}]",
                        this.SourceRequest.Context.Hash, this.Progress, this.ProgressLength);
                case RequestEvents.UploadProgress:
                    return string.Format(
                        "[RequestEventInfo Event: UploadProgress, Progress: {1}, ProgressLength: {2}, Source: {0}]",
                        this.SourceRequest.Context.Hash, this.Progress, this.ProgressLength);
                case RequestEvents.StreamingData:
                    return string.Format("[RequestEventInfo Event: StreamingData, DataLength: {1}, Source: {0}]",
                        this.SourceRequest.Context.Hash, this.DataLength);
                ...
                case RequestEvents.StateChange:
                    return string.Format("[RequestEventInfo Event: StateChange, State: {1}, Source: {0}]",
                        this.SourceRequest.Context.Hash, this.State);
                case RequestEvents.SetState:
                    return string.Format("[RequestEventInfo Event: SetState, State: {1}, Source: {0}]",
                        this.SourceRequest.Context.Hash, this.State);
                case RequestEvents.Resend:
                    return string.Format("[RequestEventInfo Event: Resend, Source: {0}]",
                        this.SourceRequest.Context.Hash);
                case RequestEvents.Headers:
                    return string.Format("[RequestEventInfo Event: Headers, Source: {0}]",
                        this.SourceRequest.Context.Hash);
    ...
    class ProgressFlattener {
        ...
        public void InsertOrUpdate(RequestEventInfo info, OnProgressDelegate onProgress) {
            ...
            var newProgress = new FlattenedProgress {
                request = info.SourceRequest, progress = info.Progress, length = info.ProgressLength,
            ...
            for (int i = 0; i < progresses.Length; ++i) {
                ...
                if (source != null && @event.onProgress != null) {
                    try {
                        ...
                        HTTPManager.Logger.Exception("ProgressFlattener", "DispatchProgressCallbacks", ex,
                            source.Context);
                    }
                }
            }
...
#pragma warning restore
        ...
        public static void EnqueueRequestEvent(RequestEventInfo ev) {
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Information("RequestEventHelper", "Enqueue " + ev.ToString(),
                    ev.SourceRequest.Context);
            ...
            while (requestEventQueue.TryDequeue(out requestEvent)) {
                ...
                switch (requestEvent.Event) {
                    case RequestEvents.DownloadProgress:
                        try {
                            if (source.DownloadSettings.OnDownloadProgress != null) {
                                ...
                                downloadProgress.InsertOrUpdate(requestEvent,
                                    source.DownloadSettings.OnDownloadProgress);
                            }
                        }
                        catch (Exception ex) {
                    ...
                    case RequestEvents.DownloadStarted:
                        try {
                            ...
                            if (response != null && downStream != null)
                                source.DownloadSettings.OnDownloadStarted?.Invoke(source, response,
                                    response.DownStream);
                        }
                        catch (Exception ex) {
                    ...
                    case RequestEvents.SetState:
                        ...
                        if (source.State >= HTTPRequestStates.Finished &&
                            requestEvent.State >= HTTPRequestStates.Finished)
                            continue;
                        ...
                        if (requestEvent.State > HTTPRequestStates.Finished) {
                            HTTPManager.Logger.Information("RequestEventHelper",
                                $"{requestEvent.State}: discarding response!",
                                source.Response?.Context ?? source.Context);
                        ...
                        if (requestEvent.State > HTTPRequestStates.Finished &&
                            requestEvent.State != HTTPRequestStates.Aborted) {
                            if (HTTPManager.LocalCache != null && !source.DownloadSettings.DisableCache) {
                                ...
                                if (HTTPManager.LocalCache.CanServeWithoutValidation(hash,
                                        ErrorTypeForValidation.ConnectionError, source.Context)) {
                    ...
                    case RequestEvents.StateChange:
                        try {
                            using (var _ = new Unity.Profiling.ProfilerMarker(
                                       nameof(RequestEventHelper.HandleRequestStateChange)).Auto())
        ...
        static readonly string[] RequestStateNames = new string[]
            {"Initial", "Queued", "Processing", "Finished", "Error", "Aborted", "ConnectionTimedOut", "TimedOut"};
        ...
        private static void HandleRequestStateChange(RequestEventInfo @event) {
            ...
            switch (@event.State) {
                ...
                case HTTPRequestStates.Aborted:
                    if (HTTPManager.Logger.IsDiagnostic)
                        HTTPManager.Logger.Information("RequestEventHelper", $"{@event.State}: discarding response!",
                            source.Response?.Context ?? source.Context);
                ...
                case HTTPRequestStates.Finished:
                    ...
                    if (source.Callback == null) {
                        ...
                        if (HTTPManager.Logger.IsDiagnostic)
                            HTTPManager.Logger.Information("RequestEventHelper",
                                $"{nameof(OnDelayedDisposeTimer)} - disposing response!", source.Context);
                        source.Dispose();
                    }
