...
namespace Best.HTTP.Hosts.Connections {
    ...
    public sealed class HTTPOverTCPConnection : ConnectionBase, INegotiationPeer {
        public PeekableContentProviderStream TopStream {
            get => this._negotiator.Stream;
        }

        public TCPStreamer Streamer {
            get => this._negotiator.Streamer;
        }
        ...
        /// </summary>
        public override int AssignedRequests {
            get => this.requestHandler != null ? this.requestHandler.AssignedRequests : base.AssignedRequests;
        }

        /// <summary>
        /// Maximum number of assignable requests.
        /// </summary>
        public override int MaxAssignedRequests {
            get => this.requestHandler != null ? this.requestHandler.MaxAssignedRequests : base.MaxAssignedRequests;
        }
        ...
        internal override void Process(HTTPRequest request) {
            ...
            if (this.requestHandler == null) {
                try {
                    ...
                    HTTPManager.Logger.Exception(nameof(HTTPOverTCPConnection), $"Process({request})", ex,
                        this.Context);
        ...
        List<string> INegotiationPeer.GetSupportedProtocolNames(Negotiator negotiator) {
            ...
            if (protocol == SupportedProtocols.HTTP &&
                negotiator.Parameters.hostSettings.HTTP2ConnectionSettings.EnableHTTP2Connections) {
        ...
        bool INegotiationPeer.MustStopAdvancingToNextStep(Negotiator negotiator, NegotiationSteps finishedStep,
        ...
        void INegotiationPeer.EvaluateProxyNegotiationFailure(Negotiator negotiator, Exception error,
        ...
        void INegotiationPeer.OnNegotiationFinished(Negotiator negotiator, PeekableContentProviderStream stream,
            TCPStreamer streamer, string negotiatedProtocol) {
        ...
        private void OnCancellationRequested(HTTPRequest req) {
            HTTPManager.Logger.Information(nameof(HTTPOverTCPConnection), $"{nameof(OnCancellationRequested)}({req})",
                this.Context);
        ...
        private bool PreprocessRequestState(Exception error) {
            ...
            HTTPManager.Logger.Information(nameof(HTTPOverTCPConnection),
                $"PreprocessRequestState({CurrentRequest}, {error})", this.Context);
        ...
        bool TrySetErrorState(HTTPRequest request, Exception ex) {
            ...
            if (ex != null) {
                ...
                ConnectionHelper.EnqueueEvents(this, HTTPConnectionStates.Closed, request, HTTPRequestStates.Aborted,
                    null);
                return true;
            }
        ...
        void StartWithNegotiatedProtocol(string negotiatedProtocol, PeekableContentProviderStream stream) {
            ...
            HTTPManager.Logger.Information(nameof(HTTPOverTCPConnection),
                $"Negotiated protocol through ALPN: '{negotiatedProtocol}'", this.Context);
            ...
            switch (negotiatedProtocol) {
                case HTTPProtocolFactory.W3C_HTTP1:
                    ...
                    useShortLivingThread = HTTPManager.PerHostSettings.Get(this.HostKey.Host).HTTP1ConnectionSettings
                                               .ForceUseThreadPool ||
                ...
                default:
                    HTTPManager.Logger.Error(nameof(HTTPOverTCPConnection),
                        $"Unknown negotiated protocol: {negotiatedProtocol}", this.Context);
