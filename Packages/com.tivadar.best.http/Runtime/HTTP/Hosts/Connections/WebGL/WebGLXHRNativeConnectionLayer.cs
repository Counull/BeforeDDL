...
namespace Best.HTTP.Hosts.Connections.WebGL
{
    internal static class WebGLXHRNativeConnectionLayer
    {
        ...
        static void OnResponse(int nativeId, [MarshalAs(UnmanagedType.LPArray, ArraySubType =
 UnmanagedType.U1, SizeParamIndex = 2)] byte[] pBuffer, int length)
        ...
        public static void OnBufferCallback(int nativeId, [MarshalAs(UnmanagedType.LPArray, ArraySubType =
 UnmanagedType.U1, SizeParamIndex = 2)] byte[] pBuffer, int length)
        {
            WebGLXHRConnection conn = null;
            if (!Connections.TryGetValue(nativeId, out conn))
