        ...
        public override string ToString() {
            switch (this.Event) {
                case ConnectionEvents.StateChange:
                    return $"[ConnectionEventInfo Source: {this.Source.ToString()} To State: {this.State}]";
                case ConnectionEvents.ProtocolSupport:
                    return
                        $"[ConnectionEventInfo Source: {this.Source.ToString()} ProtocolSupport: {this.ProtocolSupport}]";
                default:
                    return string.Format(
                        "[ConnectionEventInfo SourceConnection: {0}, Event: {1}, State: {2}, ProtocolSupport: {3}]",
...
#pragma warning restore
        ...
        public static void EnqueueConnectionEvent(ConnectionEventInfo @event) {
            if (HTTPManager.Logger.Level == Loglevels.All)
                HTTPManager.Logger.Information("ConnectionEventHelper", "Enqueue " + @event.ToString(),
            ...
            while (connectionEventQueue.TryDequeue(out connectionEvent)) {
                ...
                if (connectionEvent.Source.LastProcessedUri == null) {
                    HTTPManager.Logger.Information("ConnectionEventHelper",
                        String.Format("Ignoring ConnectionEventInfo({0}) because its LastProcessedUri is null!",
                            connectionEvent.ToString()), connectionEvent.Source.Context);
                    return;
                }
        ...
        private static void HandleConnectionStateChange(ConnectionEventInfo @event) {
            try {
                ...
                HTTPManager.Logger.Exception("ConnectionEvents", $"HandleConnectionStateChange ({@event.State})", ex,
                    @event.Source.Context);
            }
        }
    }
