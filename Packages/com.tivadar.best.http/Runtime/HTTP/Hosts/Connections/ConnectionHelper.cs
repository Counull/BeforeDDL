...
namespace Best.HTTP.Hosts.Connections {
    ...
    public static class ConnectionHelper {
        ...
        public static void EnqueueEvents(ConnectionBase connection, HTTPConnectionStates connectionState,
            HTTPRequest request, HTTPRequestStates requestState, Exception error) {
            // SetState
        ...
        public static Exception HandleResponse(HTTPRequest request,
            ...
            switch (resp.StatusCode) {
                ...
                case PermanentRedirect: // http://tools.ietf.org/html/rfc7238
                {
                    ...
                    if (!string.IsNullOrEmpty(location)) {
                        ...
                        if (HTTPManager.Logger.IsDiagnostic)
                            HTTPManager.Logger.Verbose(nameof(ConnectionHelper),
                                $"Redirected to Location: '{location}' redirectUri: '{redirectUri}'", loggingContext);

                        if (redirectUri == request.CurrentUri) {
                            HTTPManager.Logger.Information(nameof(ConnectionHelper), "Redirected to the same location!",
                                loggingContext);
                            goto default;
                        }
                        ...
                        if (!request.RedirectSettings.CallOnBeforeRedirection(request, resp, redirectUri)) {
                            HTTPManager.Logger.Information(nameof(ConnectionHelper),
                                "OnBeforeRedirection returned False", loggingContext);
                            goto default;
                        }
                        ...
                        if (!request.CurrentUri.Host.Equals(redirectUri.Host, StringComparison.OrdinalIgnoreCase)) {
                            ...
                            Shared.PlatformSupport.Network.DNS.Cache.DNSCache.Query(
                ...
                case var statusCode when statusCode == InternalServerError ||
                    ...
                    if (HTTPManager.LocalCache != null) {
                        ...
                        if (HTTPManager.LocalCache.CanServeWithoutValidation(hash, ErrorTypeForValidation.ServerError,
                                request.Context)) {
                            HTTPManager.LocalCache.Redirect(request, hash);
                            resendRequest = true;
                        }
            ...
            bool tryToKeepAlive = !string.IsNullOrEmpty(currentUri.Host) && HTTPManager.PerHostSettings
                .Get(currentUri.Host).HTTP1ConnectionSettings.TryToReuseConnections;
            ...
            if (resendRequest) {
                HTTPManager.Logger.Verbose(nameof(ConnectionHelper), "HandleResponse - discarding response",
                    request.Response?.Context ?? loggingContext);
