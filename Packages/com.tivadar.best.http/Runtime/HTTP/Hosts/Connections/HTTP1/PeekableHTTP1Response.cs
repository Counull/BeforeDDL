...
namespace Best.HTTP.Hosts.Connections.HTTP1 {
    ...
    public class PeekableHTTP1Response : HTTPResponse {
        public PeekableReadState ReadState {
            ...
            private set {
                if (this._readState != value)
                    HTTPManager.Logger.Information(nameof(PeekableHTTP1Response), $"{this._readState} => {value}",
                        this.Context);
        ...
        public PeekableHTTP1Response(HTTPRequest request, bool isFromCache,
            IDownloadContentBufferAvailable bufferAvailableHandler)
        ...
        public void ProcessPeekable(PeekableContentProviderStream peekable) {
            ...
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(PeekableHTTP1Response),
                    $"ProcessPeekable({this.ReadState}, {peekable.Length})", this.Context);
            ...
            try {
                ...
                if (peekable == null && ReadState != PeekableReadState.Content &&
                ...
                switch (ReadState) {
                    case PeekableReadState.StatusLine:
                        ...
                        if (statusCodeStr.Length > 0 && (byte) statusCodeStr[statusCodeStr.Length - 1] != LF &&
                            ...
                            HTTPManager.Logger.Warning(nameof(PeekableHTTP1Response),
                                "Skipping Status Message reading!", this.Context);
                        ...
                        if (HTTPManager.Logger.IsDiagnostic)
                            HTTPManager.Logger.Verbose(nameof(PeekableHTTP1Response),
                                $"HTTP/'{this.HTTPVersion}' '{this.StatusCode}' '{this.Message}'", this.Context);
                    ...
                    case PeekableReadState.Headers:
                        ...
                        if (this.ReadState == PeekableReadState.PrepareForContent) {
                            ...
                            if (this.StatusCode == 101 &&
                                (HasHeaderWithValue("connection", "upgrade") || HasHeader("upgrade")) &&
                                this.Request?.DownloadSettings?.OnUpgraded != null) {
                                HTTPManager.Logger.Information(nameof(PeekableHTTP1Response), "Request Upgraded!",
                                    this.Context);

                                this.IsUpgraded =
                                    this.Request.DownloadSettings.OnUpgraded(this.Request, this, peekable);
                    ...
                    case PeekableReadState.PrepareForContent:
                        ...
                        HTTPManager.Logger.Information(nameof(PeekableHTTP1Response),
                            $"PrepareForContent - delivery mode selected: {this._deliveryMode}, {this._expectedLength}!",
                            this.Context);
                        ...
                        this.sendProgressChanged =
                            this.Request.DownloadSettings.OnDownloadProgress != null && this.IsSuccess;
                    ...
                    case PeekableReadState.Content:
                        ...
                        switch (this._deliveryMode) {
                            case ContentDeliveryMode.Raw:
                                ProcessReadRaw(peekable);
                                break;
                            case ContentDeliveryMode.RawUnknownLength:
                                ProcessReadRawUnknownLength(peekable);
                                break;
                            case ContentDeliveryMode.Chunked:
                                ProcessReadChunked(peekable);
                                break;
                        }
        ...
        private void ProcessReadHeaders(PeekableStream peekable, PeekableReadState targetState) {
            ...
            do {
                ...
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(PeekableHTTP1Response), $"Header - '{headerName}': '{value}'",
                        this.Context);
        ...
        private void ProcessReadRawUnknownLength(PeekableStream peekable) {
            if (peekable == null) {
                if (sendProgressChanged)
                    RequestEventHelper.EnqueueRequestEvent(new RequestEventInfo(this.Request,
                        RequestEvents.DownloadProgress, this._downloaded, this._expectedLength));
        ...
        void PostProcessContent() {
            if (this._compressed) {
                var (decompressed, release) =
                    this._decompressor.Decompress(BufferSegment.Empty, true, true, this.Context);
                if (decompressed != BufferSegment.Empty)
                    FeedDownloadedContentChunk(decompressed);
            }
