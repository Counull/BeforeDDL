...
namespace Best.HTTP.Hosts.Connections.HTTP1 {
    public sealed class HTTP1ContentConsumer : IHTTPRequestHandler, IContentConsumer, IDownloadContentBufferAvailable,
        IThreadSignaler {
        ...
        public void RunHandler() {
            ...
            try {
                ...
                if (this._response == null)
                    this.conn.CurrentRequest.Response =
                        this._response = new PeekableHTTP1Response(this.conn.CurrentRequest, false, this);
        ...
        private void SendOutTo(HTTPRequest request, System.IO.Stream stream) {
            ...
            string requestLine = string.Format("{0} {1} HTTP/1.1", HTTPRequest.MethodNames[(byte) request.MethodType],
                requestPathAndQuery);
            ...
            using (WriteOnlyBufferedStream bufferStream = new WriteOnlyBufferedStream(stream,
                       (int) (request.UploadSettings.UploadChunkSize * 1.5f), request.Context)) {
                    ...
                    for (int i = 0; i < values.Count; ++i) {
                        if (string.IsNullOrEmpty(values[i])) {
                            HTTPManager.Logger.Warning("HTTPRequest",
                                string.Format("Null/empty value for header: {0}", header), request.Context);
                            continue;
                        }

                        if (HTTPManager.Logger.Level <= Loglevels.Information)
                            HTTPManager.Logger.Verbose("HTTPRequest", $"Header - '{header}': '{values[i]}'",
                                request.Context);
        ...
        public void OnConnectionClosed() {
            HTTPManager.Logger.Information(nameof(HTTP1ContentConsumer),
                $"OnConnectionClosed({this.ContentProvider?.Length}, {this._response?.ReadState})", this.Context);
        ...
        public void OnError(Exception e) {
            HTTPManager.Logger.Information(nameof(HTTP1ContentConsumer),
                $"OnError({this.ContentProvider?.Length}, {this._response?.ReadState}, {this.ShutdownType})",
                this.Context);
        ...
        void FinishedProcessing(Exception ex) {
            ...
            HTTPManager.Logger.Verbose(nameof(HTTP1ContentConsumer),
                $"{nameof(FinishedProcessing)}({resp.ReadState}, {ex})", this.Context);
            ...
            HTTPConnectionStates connectionState =
                ex != null ? HTTPConnectionStates.Closed : HTTPConnectionStates.Recycle;
            ...
            if (error != null) {
                ...
                error = ConnectionHelper.HandleResponse(req, out resendRequest, out connectionState,
                    ref this._keepAlive, this.Context);
            ...
            HTTPManager.Logger.Verbose(nameof(HTTP1ContentConsumer),
                $"{nameof(FinishedProcessing)} final decision. ResendRequest: {resendRequest}, RequestState: {requestState}, ConnectionState: {connectionState}",
                this.Context);

            // If HandleResponse returned with ClosedResendRequest or there were an error and we can retry the request
            if (connectionState == HTTPConnectionStates.ClosedResendRequest ||
                (resendRequest && connectionState == HTTPConnectionStates.Closed)) {
        ...
        public void Process(HTTPRequest request) {
            ...
            var useShortLivingThread = HTTPManager.PerHostSettings.Get(request.CurrentHostKey).HTTP1ConnectionSettings
                                           .ForceUseThreadPool ||
