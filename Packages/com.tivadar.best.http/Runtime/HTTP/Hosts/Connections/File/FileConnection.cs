    ...
    internal sealed class FileConnection : ConnectionBase, IContentConsumer, IDownloadContentBufferAvailable {
        ...
        protected override void ThreadFunc() {
            ...
            bool isFromLocalCache =
                this.CurrentRequest.CurrentUri.Host.Equals(HTTPCache.CacheHostName, StringComparison.OrdinalIgnoreCase);

            if (this._response == null)
                this.CurrentRequest.Response =
                    this._response = new PeekableHTTP1Response(this.CurrentRequest, isFromLocalCache, this);
            ...
            try {
                ...
                if (isFromLocalCache) {
                    ...
                    stream.AppendStream(HTTPManager.IOService.CreateFileStream(
                        HTTPManager.LocalCache.GetHeaderPathFromHash(hash), FileStreamModes.OpenRead));
                }
                else {
                    headers.WriteLine($"BestHTTP-Origin: file");
                    headers.WriteLine("Content-Type: application/octet-stream");

                    contentStream = HTTPManager.IOService.CreateFileStream(this.CurrentRequest.CurrentUri.LocalPath,
        ...
        public void OnConnectionClosed() {
            HTTPManager.Logger.Information(nameof(FileConnection),
        ...
        void FinishedProcessing(Exception ex) {
            ...
            HTTPManager.Logger.Verbose(nameof(FileConnection), $"{nameof(FinishedProcessing)}({resp}, {ex})",
                this.Context);
            ...
            if (error != null) {
                ...
                error = ConnectionHelper.HandleResponse(req, out resendRequest, out connectionState, ref keepAlive,
                    this.Context);
            ...
            HTTPManager.Logger.Verbose(nameof(FileConnection),
