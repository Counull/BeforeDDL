...
namespace Best.HTTP.Hosts.Connections.HTTP2 {
    ...
    public class HTTP2Stream : IDownloadContentBufferAvailable {
        ...
        /// </summary>
        public HTTP2Stream(UInt32 id, HTTP2ContentConsumer parentHandler, HTTP2SettingsManager registry,
        ...
        public virtual void Assign(HTTPRequest request) {
            ...
            HTTPManager.Logger.Information("HTTP2Stream",
                string.Format("[{0}] Request assigned to stream. Remote Window: {1:N0}. Uri: {2}", this.Id,
                    this.remoteWindow, request.CurrentUri.ToString()), this.Context);
#endif
        ...
        public void Process(List<HTTP2FrameHeaderAndPayload> outgoingFrames) {
            if (this.AssignedRequest.IsCancellationRequested && !this.isRSTFrameSent) {
#if ENABLE_LOGGING
                HTTPManager.Logger.Information("HTTP2Stream",
                    $"[{this.Id}] Process({this.State}) - IsCancellationRequested", this.Context);
#endif
                ...
                if (this.State != HTTP2StreamStates.Idle)
                    this.outgoing.Enqueue(
                        HTTP2FrameHelper.CreateRSTFrame(this.Id, HTTP2ErrorCodes.CANCEL, this.Context));

                // We can close the stream if already received headers, or not even sent one
                if (this.State == HTTP2StreamStates.HalfClosedRemote ||
                    this.State == HTTP2StreamStates.HalfClosedLocal || this.State == HTTP2StreamStates.Idle)
...
#if ENABLE_LOGGING
            HTTPManager.Logger.Information("HTTP2Stream",
                $"[{this.Id}] Abort(\"{msg}\", {this.State}, {this.AssignedRequest.State})", this.Context);
#endif
            ...
            if (this.State != HTTP2StreamStates.Closed) {
                ...
                if (this.AssignedRequest.State != HTTPRequestStates.Processing) {
                    ...
                    RequestEventHelper.EnqueueRequestEvent(new RequestEventInfo(this.AssignedRequest,
                        HTTPRequestStates.Error, new Exception(msg)));
        ...
        protected void ProcessIncomingFrames(List<HTTP2FrameHeaderAndPayload> outgoingFrames) {
            while (this.incomingFrames.Count > 0) {
                ...
                if ( /*HTTPManager.Logger.Level == Logger.Loglevels.All && */frame.Type != HTTP2FrameTypes.DATA &&
                    ...
                    HTTPManager.Logger.Information("HTTP2Stream",
                        string.Format("[{0}] Process - processing frame: {1}", this.Id, frame.ToString()),
                        this.Context);
#endif
                            ...
                            try {
                                ...
                                HTTPManager.Logger.Exception("HTTP2Stream",
                                    string.Format("[{0}] ProcessIncomingFrames - Header Frames: {1}, Encoder: {2}",
                                        this.Id, this.headerView.ToString(), this.encoder.ToString()), ex,
                                    this.Context);
                            }
                    ...
                    default:
                        HTTPManager.Logger.Warning("HTTP2Stream",
                            string.Format("[{0}] Unexpected frame ({1}, Payload: {2}) in state {3}!", this.Id, frame,
                                frame.PayloadAsHex(), this.State), this.Context);
                        break;
                }
            ...
            switch (this.State) {
                case HTTP2StreamStates.Idle:
                    ...
                    if (this.AssignedRequest.UploadSettings.UploadStream == null) {
                        ...
                        if (this.AssignedRequest.UploadSettings != null &&
                            this.AssignedRequest.UploadSettings.UploadStream != null)
                    ...
                    if (this.AssignedRequest.DownloadSettings.ContentStreamMaxBuffered >
                        HTTP2ContentConsumer.MaxValueFor31Bits)
                        this.AssignedRequest.DownloadSettings.ContentStreamMaxBuffered =
                            HTTP2ContentConsumer.MaxValueFor31Bits;

                    long localWindowDiff = this.AssignedRequest.DownloadSettings.ContentStreamMaxBuffered -
                                           this.localWindow;

                    if (localWindowDiff > 0) {
                        this.localWindow += localWindowDiff;
                        this.outgoing.Enqueue(HTTP2FrameHelper.CreateWindowUpdateFrame(this.Id,
                            (UInt32) localWindowDiff, this.Context));
                    }
                ...
                case HTTP2StreamStates.HalfClosedLocal:
                    if (this.response?.DownStream != null) {
                        var windowIncrement = this.response.DownStream.MaxBuffered - this.localWindow -
                                              this.response.DownStream.Length;
                        if (windowIncrement > 0) {
                            this.localWindow += windowIncrement;
                            outgoingFrames.Add(HTTP2FrameHelper.CreateWindowUpdateFrame(this.Id,
                                (UInt32) windowIncrement, this.Context));

#if ENABLE_LOGGING
                            HTTPManager.Logger.Information("HTTP2Stream",
                                $"[{this.Id}] Sending window inc. update: {windowIncrement:N0}, {this.localWindow:N0}/{this.response.DownStream.MaxBuffered:N0}",
                                this.Context);
#endif
                        }
                    }
        ...
        protected virtual void ProcessOpenState(List<HTTP2FrameHeaderAndPayload> outgoingFrames) {
...
#if ENABLE_LOGGING
                HTTPManager.Logger.Information("HTTP2Stream",
                    string.Format("[{0}] Skipping data sending as remote Window is {1}!", this.Id, this.remoteWindow),
                    this.Context);
#endif
                return;
            }
            ...
            Int64 maxFrameSize = Math.Min(this.AssignedRequest.UploadSettings.UploadChunkSize,
                Math.Min(this.remoteWindow, this.settings.RemoteSettings[HTTP2Settings.MAX_FRAME_SIZE]));
            ...
            this.lastReadCount = this.AssignedRequest.UploadSettings.UploadStream.Read(frame.Payload.Data, 0,
                (int) Math.Min(maxFrameSize, int.MaxValue));
            ...
            if (this.AssignedRequest.UploadSettings.OnUploadProgress != null)
                RequestEventHelper.EnqueueRequestEvent(new RequestEventInfo(this.AssignedRequest,
                    RequestEvents.UploadProgress, this.sentData, uploadLength));
        }

        protected void OnRemoteSettingChanged(HTTP2SettingsRegistry registry, HTTP2Settings setting, uint oldValue,
            uint newValue) {
            switch (setting) {
        ...
        protected void FinishRequest() {
            ...
            try {
                ...
                ConnectionHelper.HandleResponse(this.AssignedRequest, out resendRequest, out proposedConnectionStates,
                    ref keepAliveHeader, this.Context);
                ...
                if (resendRequest && !this.AssignedRequest.IsCancellationRequested)
                    ...
                    RequestEventHelper.EnqueueRequestEvent(new RequestEventInfo(this.AssignedRequest,
                        HTTPRequestStates.Finished, null));
                }
            }
            catch (Exception ex) {
