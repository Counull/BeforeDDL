    ...
    public sealed class HTTP2SettingsRegistry {
            ...
            for (int i = 0; i < settings.Count; ++i) {
                ...
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Information("HTTP2SettingsRegistry",
                        string.Format("Merge {0}({1}) = {2}", setting, key, value), this._parent.Context);
            }
        }
        ...
        internal HTTP2FrameHeaderAndPayload CreateFrame(LoggingContext context) {
            List<KeyValuePair<HTTP2Settings, UInt32>> keyValuePairs =
                new List<KeyValuePair<HTTP2Settings, uint>>(HTTP2SettingsManager.KnownSettingsCount);

            for (int i = 1; i < HTTP2SettingsManager.KnownSettingsCount; ++i)
                if (this.changeFlags[i]) {
                    keyValuePairs.Add(
                        new KeyValuePair<HTTP2Settings, uint>((HTTP2Settings) i, this[(HTTP2Settings) i]));
    ...
    public sealed class HTTP2SettingsManager {
        ...
        internal void Process(HTTP2FrameHeaderAndPayload frame, List<HTTP2FrameHeaderAndPayload> outgoingFrames) {
            ...
            if (HTTPManager.Logger.Level <= Loglevels.Information)
                HTTPManager.Logger.Information("HTTP2SettingsManager",
                    "Processing Settings frame: " + settingsFrame.ToString(), this.Context);
        ...
        internal void SendChanges(List<HTTP2FrameHeaderAndPayload> outgoingFrames) {
            if (this.SettingsChangesSentAt != DateTime.MinValue &&
                DateTime.Now - this.SettingsChangesSentAt >= this._connectionSettings.Timeout) {
