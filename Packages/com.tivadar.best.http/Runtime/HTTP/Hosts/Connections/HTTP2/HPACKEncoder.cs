...
namespace Best.HTTP.Hosts.Connections.HTTP2 {
    public sealed class HPACKEncoder {
        ...
        public void Encode(HTTP2Stream context, HTTPRequest request, Queue<HTTP2FrameHeaderAndPayload> to,
            ...
            using (BufferPoolMemoryStream bufferStream =
                   new BufferPoolMemoryStream(request.UploadSettings.UploadChunkSize)) {
                request.Prepare();

                // add :method
                switch (request.MethodType) {
                    case HTTPMethods.Get:
                        WriteIndexedHeaderField(bufferStream, 2);
                        break;
                    case HTTPMethods.Post:
                        WriteIndexedHeaderField(bufferStream, 3);
                        break;
                    default:
                        WriteLiteralHeaderFieldWithoutIndexing_IndexedName(bufferStream, 2,
                            HTTPRequest.MethodNames[(int) request.MethodType]);
                        break;
                }
                ...
                request.EnumerateHeaders((header, values) => {
                    if (header.Equals("connection", StringComparison.OrdinalIgnoreCase) ||
                        (header.Equals("te", StringComparison.OrdinalIgnoreCase) && !values.Contains("trailers") &&
                         values.Count <= 1) ||
                        ...
                        if (HTTPManager.Logger.Level <= Loglevels.Information)
                            HTTPManager.Logger.Information("HPACKEncoder",
                                string.Format("[{0}] - Encode - Header({1}/{2}): '{3}': '{4}'", context.Id, i + 1,
                                    values.Count, header, values[i]), this._context);
                    }
                }, true);
                        ...
                        if (HTTPManager.Logger.Level <= Loglevels.Information)
                            HTTPManager.Logger.Information("HPACKEncoder",
                                string.Format("[{0}] Decode - LiteralHeaderFieldWithIncrementalIndexing_NewName: {1}",
                                    context.Id, header.ToString()), this._context);
        ...
        private static void EncodeStringWithHuffman(string str, UInt32 encodedLength, byte[] buffer,
            ref UInt32 offset) {
            int requiredBytesForStr = System.Text.Encoding.UTF8.GetByteCount(str);
            byte[] strBytes = BufferPool.Get(requiredBytesForStr, true);
        ...
        private static void AddCodePointToBuffer(HuffmanTableEntry code, byte[] buffer, ref UInt32 offset,
