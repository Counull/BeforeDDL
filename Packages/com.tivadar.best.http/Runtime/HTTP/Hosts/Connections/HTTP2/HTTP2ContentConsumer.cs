...
namespace Best.HTTP.Hosts.Connections.HTTP2 {
    public delegate HTTP2Stream CustomHTTP2StreamFactory(HTTPRequest request, uint streamId,
        HTTP2ContentConsumer parentHandler, HTTP2SettingsManager registry, HPACKEncoder hpackEncoder);
    ...
    public sealed class HTTP2ContentConsumer : IHTTPRequestHandler, IContentConsumer, IThreadSignaler {
        ...
        public bool CanProcessMultiple {
            get { return !this.SentGoAwayFrame && this.isRunning && this._maxAssignedRequests > 1; }
        }
        ...
        private bool SentGoAwayFrame {
            get => this.goAwaySentAt != DateTime.MaxValue;
        }
        ...
        public void RunHandler() {
#if ENABLE_LOGGING
            HTTPManager.Logger.Information(nameof(HTTP2ContentConsumer), "Processing thread up and running!",
                this.Context);
#endif
            ...
            try {
                ...
                using (WriteOnlyBufferedStream bufferedStream =
                       new WriteOnlyBufferedStream(this.conn.TopStream, 1024 * 1024 /*1500 - 20 - 20*/, this.Context)) {
                    // The client connection preface starts with a sequence of 24 octets
                    // Connection preface starts with the string PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n).
                    ReadOnlySpan<byte> MAGIC = stackalloc byte[24] {
                    ...
                    this.settings.InitiatedMySettings[HTTP2Settings.INITIAL_WINDOW_SIZE] =
                        this._connectionSettings.InitialStreamWindowSize;
                    this.settings.InitiatedMySettings[HTTP2Settings.MAX_CONCURRENT_STREAMS] =
                        this._connectionSettings.MaxConcurrentStreams;
                    this.settings.InitiatedMySettings[HTTP2Settings.ENABLE_CONNECT_PROTOCOL] =
                        (uint) (this._connectionSettings.EnableConnectProtocol ? 1 : 0);
                    ...
                    if (initialConnectionWindowSize > 65535) {
                        ...
                        if (initialConnectionWindowSizeDiff > 0)
                            this.outgoingFrames.Add(HTTP2FrameHelper.CreateWindowUpdateFrame(0,
                                (UInt32) initialConnectionWindowSizeDiff, this.Context));
                    }
                    ...
                    while (this.isRunning) {
                        ...
                        if (!atLeastOneStreamHasAFrameToSend) {
                            ...
                            var timeoutAt = this.waitingForPingAck != 0
                                ? this.lastPingSent + this._connectionSettings.Timeout
                                : DateTime.MaxValue;

                            // sendPingAt can be in the past if Timeout is larger than PingFrequency
                            var nextPingInteraction =
                                sendPingAt < timeoutAt && sendPingAt >= now ? sendPingAt : timeoutAt;
                        ...
                        if (now - this.lastPingSent >= this._connectionSettings.PingFrequency &&
                            Interlocked.CompareExchange(ref this.waitingForPingAck, 1, 0) == 0) {
...
#if ENABLE_LOGGING
                            HTTPManager.Logger.Information(nameof(HTTP2ContentConsumer),
                                $"PING frame created with payload: {frame.Payload.Slice(0, 8)}", this.Context);
#endif
                            ...
                            if (header.StreamId > 0) {
                                ...
                                if (http2Stream != null) {
                                    ...
                                    if (HTTPManager.Logger.Level == Loglevels.All)
                                        HTTPManager.Logger.Warning(nameof(HTTP2ContentConsumer),
                                            $"Can't deliver frame: {header}, because no stream could be found for its Id!",
                                            this.Context);
#endif
                                ...
                                switch (header.Type) {
                                    ...
                                    case HTTP2FrameTypes.PING:
                                        ...
                                        if ((pingFrame.Flags & HTTP2PingFlags.ACK) != 0) {
                                            ...
                                            if (diff.TotalSeconds > 10 || diff.TotalSeconds < 0)
                                                HTTPManager.Logger.Warning(nameof(HTTP2ContentConsumer),
                                                    $"Pong received with weird diff: {diff}! Payload: {pingFrame.OpaqueData}",
                                                    this.Context);
#endif
...
#if ENABLE_LOGGING
                                            HTTPManager.Logger.Verbose(nameof(HTTP2ContentConsumer),
                                                string.Format("Latency: {0:F2}ms, RTT buffer: {1}", this.Latency,
                                                    this.rtts.ToString()), this.Context);
#endif
                                        }
                                            ...
                                            var frame = HTTP2FrameHelper.CreatePingFrame(HTTP2PingFlags.ACK,
                                                this.Context);
                                            Array.Copy(pingFrame.OpaqueData.Data, 0, frame.Payload.Data, 0,
                                                pingFrame.OpaqueData.Count);
                                    ...
                                    case HTTP2FrameTypes.GOAWAY:
                                        ...
                                        abortWithMessage = string.Format(
                                            "Server closing the connection! Error code: {0} ({1}) Additonal Debug Data: {2}",
                        ...
                        if (this.clientInitiatedStreams.Count < _maxAssignedRequests && this.isRunning) {
                            ...
                            HTTPRequest request;
                            while (this.clientInitiatedStreams.Count < _maxAssignedRequests &&
                                   this.requestQueue.TryDequeue(out request)) {
                                HTTP2Stream newStream = null;

                                if (request.Tag is CustomHTTP2StreamFactory factory) {
                                    newStream = factory(request, (UInt32) Interlocked.Add(ref LastStreamId, 2), this,
                                        this.settings, this.HPACKEncoder);
                                }
                                else {
                                    newStream = new HTTP2Stream((UInt32) Interlocked.Add(ref LastStreamId, 2), this,
                                        this.settings, this.HPACKEncoder);
                                }
                        ...
                        if (this.ShutdownType == ShutdownTypes.Running && !this.SentGoAwayFrame &&
                            now - this.lastInteraction >= this._connectionSettings.MaxIdleTime) {
                            this.lastInteraction = now;
#if ENABLE_LOGGING
                            HTTPManager.Logger.Information(nameof(HTTP2ContentConsumer),
                                "Reached idle time, sending GoAway frame!", this.Context);
#endif
                            this.outgoingFrames.Add(
                                HTTP2FrameHelper.CreateGoAwayFrame(0, HTTP2ErrorCodes.NO_ERROR, this.Context));
                        ...
                        if (localConnectionWindow < updateConnectionWindowAt) {
                            ...
                            if (HTTPManager.Logger.IsDiagnostic)
                                HTTPManager.Logger.Information(nameof(HTTP2ContentConsumer),
                                    $"Updating local connection window by {diff:N0} ({initialConnectionWindowSize:N0} - {localConnectionWindow:N0})",
                                    this.Context);
#endif
                        ...
                        for (int i = 0; i < this.outgoingFrames.Count; ++i) {
                            ...
                            if (HTTPManager.Logger.IsDiagnostic &&
                                ...
                                HTTPManager.Logger.Information(nameof(HTTP2ContentConsumer),
                                    "Sending frame: " + frame.ToString(), this.Context);
#endif
                            ...
                            switch (frame.Type) {
                                case HTTP2FrameTypes.DATA:
                                    ...
                                    if (frame.Payload.Count > this.remoteWindow) {
                                        ...
                                        HTTPManager.Logger.Warning(nameof(HTTP2ContentConsumer),
                                            string.Format(
                                                "Data sending halted for this round. Remote Window: {0:N0}, frame: {1}",
                                                this.remoteWindow, frame.ToString()), this.Context);
#endif
                                        continue;
                                    }
                ...
                if (this.ShutdownType == ShutdownTypes.Running && this.isRunning && !this.SentGoAwayFrame &&
...
#if ENABLE_LOGGING
                HTTPManager.Logger.Information(nameof(HTTP2ContentConsumer),
                    $"Sender thread closing - cleaning up remaining requests({this.clientInitiatedStreams.Count})...",
                    this.Context);
#endif
...
#if ENABLE_LOGGING
                    if (HTTPManager.Logger
                        .IsDiagnostic /*&& header.Type != HTTP2FrameTypes.DATA /*&& header.Type != HTTP2FrameTypes.PING*/
                       )
                        HTTPManager.Logger.Information(nameof(HTTP2ContentConsumer),
                            "New frame received: " + header.ToString(), this.Context);
#endif
...
#if ENABLE_LOGGING
            HTTPManager.Logger.Verbose(nameof(HTTP2ContentConsumer), $"{nameof(OnConnectionClosed)}({this.isRunning})",
                this.Context);
#endif
            this.isRunning = false;
            this.newFrameSignal?.Set();
...
#if ENABLE_LOGGING
            HTTPManager.Logger.Exception(nameof(HTTP2ContentConsumer), $"{nameof(OnError)}({this.isRunning}, {ex})", ex,
                this.Context);
#endif
            this.isRunning = false;
            this.newFrameSignal?.Set();
...
#if ENABLE_LOGGING
                HTTPManager.Logger.Information(nameof(HTTP2ContentConsumer),
                    string.Format("Dispose - Request '{0}' IsCancellationRequested: {1}", request.CurrentUri.ToString(),
                        request.IsCancellationRequested.ToString()), this.Context);
#endif
                RequestEventHelper.EnqueueRequestEvent(request.IsCancellationRequested
                    ? new RequestEventInfo(request, HTTPRequestStates.Aborted, null)
                    : new RequestEventInfo(request, RequestEvents.Resend));
            }
