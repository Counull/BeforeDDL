    ...
    public class HostVariant {
        ...
        public virtual void AddProtocol(HostProtocolSupport protocolSupport) {
            ...
            if (oldProtocol != protocolSupport) {
                ...
                HTTPManager.Logger.Information(nameof(HostVariant), $"AddProtocol({oldProtocol} => {protocolSupport})",
                    this.Context);
            }
        ...
        public virtual HostVariant TryToSendQueuedRequests() {
            ...
            (int activeConnections, int theoreticalMaximumPerConnection, int assignedRequests) =
                QueryAnyAvailableOrNew(ref availableConnections);

            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(HostVariant),
                    $"QueryAnyAvailableOrNew => ({activeConnections}, {theoreticalMaximumPerConnection}, {assignedRequests}), available: {availableConnections.Count}, protocol: {this.ProtocolSupport}, max connections: {this._settings.MaxConnectionPerVariant}",
                    this.Context);
            ...
            while (this.Queue.Count > 0 && availableConnections.Count > 0) {
                ...
                if (nextRequest.State <= HTTPRequestStates.Queued) {
                    ...
                    if (HTTPManager.Logger.IsDiagnostic)
                        HTTPManager.Logger.Information(nameof(HostVariant), $"Send({kvp.Value.GetType().Name})",
                            nextRequest.Context);

                    RequestEventHelper.EnqueueRequestEvent(new RequestEventInfo(nextRequest,
                        HTTPRequestStates.Processing, null));
        ...
        public virtual (int activeConnections, int theoreticalMaximumPerConnection, int assignedRequests)
            QueryAnyAvailableOrNew(ref List<KeyValuePair<int, ConnectionBase>> connectionCollector) {
            ...
            return (activeConnections,
                Math.Max(1, (int) (maxAssignedRequest * this._settings.MaxAssignedRequestsFactor)), assignedRequests);
        }
        ...
        public virtual HostVariant RecycleConnection(ConnectionBase conn) {
            ...
            Best.HTTP.Shared.Extensions.Timer.Add(new TimerData(TimeSpan.FromSeconds(1), conn,
        ...
        protected virtual bool RemoveConnectionImpl(ConnectionBase conn, HTTPConnectionStates setState) {
            HTTPManager.Logger.Information(typeof(HostVariant).Name, $"RemoveConnectionImpl({conn}, {setState})",
                this.Context);
            ...
            if (!found) // 
                HTTPManager.Logger.Information(typeof(HostVariant).Name,
        ...
        protected virtual bool CloseConnectionAfterInactivity(DateTime now, object context) {
            ...
            bool closeConnection = conn.State == HTTPConnectionStates.Free &&
                                   now - conn.LastProcessTime >= conn.KeepAliveTime;
            if (closeConnection) {
                HTTPManager.Logger.Information(typeof(HostVariant).Name, string.Format(
                    ...
                    conn.LastProcessTime.ToString(System.Globalization.CultureInfo.InvariantCulture),
                    conn.KeepAliveTime), this.Context);
        ...
        public virtual void LoadFrom(int version, System.IO.BinaryReader br) {
            ...
            if (DateTime.Now - this.LastProtocolSupportUpdate >= TimeSpan.FromDays(1)) {
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(HostVariant),
                        $"LoadFrom - Too Old! LastProtocolSupportUpdate: {this.LastProtocolSupportUpdate.ToString(CultureInfo.InvariantCulture)}, ProtocolSupport: {this.ProtocolSupport}",
                        this.Context);
                this.ProtocolSupport = HostProtocolSupport.Unknown;
            }
            else if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(HostVariant),
                    $"LoadFrom - LastProtocolSupportUpdate: {this.LastProtocolSupportUpdate.ToString(CultureInfo.InvariantCulture)}, ProtocolSupport: {this.ProtocolSupport}",
                    this.Context);
        }

        public override string ToString() =>
            $"{this.Host}, {this.Queue.Count}/{this.Connections?.Count}, {this.ProtocolSupport}";
    }
}