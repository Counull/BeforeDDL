...
namespace Best.HTTP.Caching {
    ...
    public delegate void OnBeforeBeginCacheDelegate(HTTPMethods method, Uri uri, int statusCode,
        Dictionary<string, List<string>> headers, LoggingContext context = null);
    ...
    public class HTTPCache : IDisposable, IHeartbeat {
        ...
        public void SetupValidationHeaders(HTTPRequest request) {
            ...
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Information(nameof(HTTPCache),
                    $"{nameof(SetupValidationHeaders)}({request}, {hash})", request.Context);
        ...
        public HTTPCacheContentWriter BeginCache(HTTPMethods method, Uri uri, int statusCode,
            Dictionary<string, List<string>> headers, LoggingContext context) {
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Information(nameof(HTTPCache),
                    $"{nameof(BeginCache)}({method}, {uri}, {statusCode}, {headers?.Count})", context);
            ...
            if (onBeforeBeginCache != null) {
                try {
                    HTTPManager.Logger.Information(nameof(HTTPCache),
                        $"{nameof(BeginCache)} - Calling {nameof(OnBeforeBeginCache)}", context);
            ...
            if (ulong.TryParse(contentLengthStr, out var contentLength)) {
                if (!IsThereEnoughSpaceAfterMaintain(contentLength, context)) {
                    HTTPManager.Logger.Information(nameof(HTTPCache),
                        $"{nameof(BeginCache)} - Not enough space({contentLength:N0}) in cache({CacheSize:N0}), even after Maintain!",
                        context);
                    return null;
                }
            }
            ...
            if (!_database.TryAcquireWriteLock(hash, headers, context)) {
                HTTPManager.Logger.Information(nameof(HTTPCache),
                    $"{nameof(BeginCache)} - Couldn't acquire write lock!", context);
                return null;
            }
            ...
            try {
                ...
                using (var headStream =
                       HTTPManager.IOService.CreateFileStream(GetHeaderPathFromHash(hash), FileStreamModes.Create))
                    WriteHeaders(headStream, headers);

                // Create/open the content cache file.
                contentStream =
                    HTTPManager.IOService.CreateFileStream(GetContentPathFromHash(hash), FileStreamModes.Create);
            }
            catch (Exception ex) {
        ...
        public void EndCache(HTTPCacheContentWriter cacheResult, bool completedWithoutIssue, LoggingContext context) {
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Information(nameof(HTTPCache),
                    $"{nameof(EndCache)}({cacheResult}, {completedWithoutIssue})", context);
        ...
        public bool RefreshHeaders(Hash128 hash, Dictionary<string, List<string>> newHeaders, LoggingContext context) {
            ...
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Information(nameof(HTTPCache),
                    $"{nameof(RefreshHeaders)}({hash}, {newHeaders?.Count})", context);
            ...
            if (_database.Update(hash, newHeaders, context)) {
                ...
                try {
                    using (var headerStream = HTTPManager.IOService.CreateFileStream(GetHeaderPathFromHash(hash),
                    ...
                    HTTPManager.Logger.Warning(nameof(HTTPCache),
                        $"{nameof(RefreshHeaders)} - Couldn't merge/store headers. Exception: {ex}", context);
        ...
        public bool CanServeWithoutValidation(Hash128 hash, ErrorTypeForValidation errorType, LoggingContext context) {
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Information(nameof(HTTPCache),
                    $"{nameof(CanServeWithoutValidation)}({hash}, {errorType})", context);
            ...
            try {
                ...
                HTTPManager.Logger.Exception(nameof(HTTPCache), $"{nameof(_database.FindContentAndMetadataLocked)}", ex,
                    context);
            ...
            if (content.MaxAge > 0) {
                ...
                switch (errorType) {
                    case ErrorTypeForValidation.None:
                        ...
                        if (current_age > maxAge && content.StaleWhileRevalidate > 0 &&
                            current_age <= (maxAge + content.StaleWhileRevalidate)) {
        ...
        public void Redirect(HTTPRequest request, Hash128 hash) {
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Information(nameof(HTTPCache), $"{nameof(Redirect)}({request}, {hash})",
                    request.Context);
        ...
        internal void Maintain(ulong contentLength, bool deleteLockedEntries, LoggingContext context) {
            ...
            HTTPManager.Logger.Information(nameof(HTTPCache),
                $"Maintain({contentLength:N0}, {deleteLockedEntries}, {System.Threading.Thread.CurrentThread.ManagedThreadId})",
                context);

            if (HTTPUpdateDelegator.Instance.IsMainThread())
                ThreadedRunner.RunShortLiving<ulong, bool, DateTime, LoggingContext>(MaintainImplementation,
                    contentLength, deleteLockedEntries, HTTPManager.CurrentFrameDateTime, context);
            else
        ...
        private void MaintainImplementation(ulong contentLength, bool deleteLockedEntries, DateTime now,
            ...
            ZeroOutCacheSize();
            try {
                var deleteOlderDT = Options.DeleteOlder == TimeSpan.MaxValue
                    ? DateTime.MinValue
                    : now - Options.DeleteOlder;
                ...
                for (int i = 0; i < _database.MetadataService.Metadatas.Count; ++i) {
                    ...
                    if (isIncomplete)
                        HTTPManager.Logger.Warning(nameof(HTTPCache), $"Incomplete cache entry({metadata}) found!",
                            context);
            ...
            if (markedForDelete != null) {
                HTTPManager.Logger.Information(nameof(HTTPCache),
                    $"Maintain - collected {markedForDelete.Count} entries for deletion!", context);
                ...
                HTTPManager.Logger.Information(nameof(HTTPCache), "Maintain - collected 0 entries for deletion!",
                    context);
        }
