...
namespace Best.HTTP.Caching {
    ...
    public class HTTPCacheContentWriter {
        ...
        internal HTTPCacheContentWriter(HTTPCache cache, Hash128 hash, Stream contentStream, ulong expectedLength,
            ...
            try {
                ...
                if (this.ProcessedLength > this.ExpectedLength) {
                    if (!this.Cache.IsThereEnoughSpaceAfterMaintain(this.ProcessedLength, this.Context)) {
                        HTTPManager.Logger.Information(nameof(HTTPCacheContentWriter),
                            $"Not enough space({this.ProcessedLength:N0}) in cache({this.Cache.CacheSize:N0}), even after Maintain!",
                            this.Context);
                ...
                HTTPManager.Logger.Warning(nameof(HTTPCacheContentWriter), $"{nameof(Write)}({segment}): {ex}",
                    this.Context);
