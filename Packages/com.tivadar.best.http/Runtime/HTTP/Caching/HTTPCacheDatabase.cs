...
namespace Best.HTTP.Caching {
    ...
    internal class CacheMetadata : Metadata {
        ...
        public unsafe override void SaveTo(Stream stream) {
            ...
            stream.EncodeSignedVariableByteInteger(LastAccessTime.ToBinary() >>
                                                   CacheMetadataContentParser.PrecisionShift);
        ...
        public unsafe override void LoadFrom(Stream stream) {
            ...
            this.LastAccessTime = DateTime.FromBinary(stream.DecodeSignedVariableByteInteger() <<
                                                      CacheMetadataContentParser.PrecisionShift);
                    ...
                    for (int i = 0; i < parser.Values.Count; ++i) {
                        ...
                        switch (kvp.Key.ToLowerInvariant()) {
                            ...
                            case "must-revalidate":
                                this.Flags |= CacheFlags.MustRevalidate;
                                break;
                            // https://www.rfc-editor.org/rfc/rfc9111.html#name-no-cache-2
                            case "no-cache":
                                this.Flags |= CacheFlags.NoCache;
                                break;
                            // https://www.rfc-editor.org/rfc/rfc5861.html#section-3
                            case "stale-while-revalidate":
                                this.StaleWhileRevalidate = kvp.HasValue ? kvp.Value.ToUInt32(0) : 0;
                                break;
                            // https://www.rfc-editor.org/rfc/rfc5861.html#section-4
                            case "stale-if-error":
                                this.StaleIfError = kvp.HasValue ? kvp.Value.ToUInt32(0) : 0;
                                break;
                        }
                    }
                }
    ...
    internal sealed class CacheMetadataIndexingService : IndexingService<CacheMetadataContent, CacheMetadata> {
        private AVLTree<UnityEngine.Hash128, int> index_Hash =
            new AVLTree<UnityEngine.Hash128, int>(new Hash128Comparer());
    ...
    internal sealed class CacheMetadataService : MetadataService<CacheMetadata, CacheMetadataContent> {
        public CacheMetadataService(IndexingService<CacheMetadataContent, CacheMetadata> indexingService,
            IEmptyMetadataIndexFinder<CacheMetadata> emptyMetadataIndexFinder)
        ...
        public CacheMetadata Create(UnityEngine.Hash128 hash, CacheMetadataContent value, int filePos, int length) {
            var result = base.CreateDefault(value, filePos, length, (content, metadata) => { metadata.Hash = hash; });
    ...
    internal sealed class HTTPCacheDatabase : Database<CacheMetadataContent, CacheMetadata, CacheMetadataIndexingService
        ...
        private HTTPCacheDatabase(string directory,
            ...
            : base(directory, options, indexingService, new CacheMetadataContentParser(),
                new CacheMetadataService(indexingService, new FindDeletedMetadataIndexFinder<CacheMetadata>())) { }

        public CacheMetadataContent FindByHashAndUpdateRequestTime(UnityEngine.Hash128 hash, LoggingContext context) {
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(HTTPCacheDatabase),
                    $"{nameof(FindByHashAndUpdateRequestTime)}({hash})", context);
        ...
        public bool TryAcquireWriteLock(Hash128 hash, Dictionary<string, List<string>> headers,
            LoggingContext context) {
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(HTTPCacheDatabase),
                    $"{nameof(TryAcquireWriteLock)}({hash}, {headers?.Count})", context);
            ...
            if (metadata != null) {
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(HTTPCacheDatabase),
                        $"{nameof(TryAcquireWriteLock)} - Metadata found: {metadata}", context);
                ...
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(HTTPCacheDatabase),
                        $"{nameof(TryAcquireWriteLock)} - Creating new DB entry", context);
        ...
        public bool Update(Hash128 hash, Dictionary<string, List<string>> headers, LoggingContext context) {
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(HTTPCacheDatabase), $"{nameof(Update)}({hash}, {headers?.Count})",
                    context);
        ...
        public void ReleaseWriteLock(Hash128 hash, ulong length, LoggingContext context) {
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(HTTPCacheDatabase),
                    $"{nameof(ReleaseWriteLock)}({hash}, {length:N0})", context);
            ...
            var (content, metadata) = FindContentAndMetadata(hash);
            if (content == null) {
                HTTPManager.Logger.Warning(nameof(HTTPCacheDatabase),
                    $"{nameof(ReleaseWriteLock)} - Couldn't find content!", context);
                return;
            }

            if (metadata.Lock != LockTypes.Write)
                HTTPManager.Logger.Error(nameof(HTTPCacheDatabase),
                    $"{nameof(ReleaseWriteLock)} - Is NOT Write Locked! {metadata}", context);
        ...
        public bool TryAcquireReadLock(Hash128 hash, LoggingContext context) {
            ...
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(HTTPCacheDatabase), $"{nameof(TryAcquireReadLock)} - {metadata}",
                    context);
        ...
        public void ReleaseReadLock(Hash128 hash, LoggingContext context) {
            ...
            if (metadata == null) {
                HTTPManager.Logger.Warning(nameof(HTTPCacheDatabase),
                    $"{nameof(ReleaseReadLock)} - Couldn't find metadata!", context);
                return;
            }

            if (metadata.Lock != LockTypes.Read)
                HTTPManager.Logger.Warning(nameof(HTTPCacheDatabase), $"{nameof(ReleaseReadLock)} - Is NOT Locked!",
                    context);

            if (metadata.ReadLockCount == 0)
                HTTPManager.Logger.Error(nameof(HTTPCacheDatabase),
                    $"{nameof(ReleaseReadLock)} - ReadLockCount already zero!", context);

            if (--metadata.ReadLockCount == 0)
                metadata.Lock = LockTypes.Unlocked;

            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(HTTPCacheDatabase), $"{nameof(ReleaseReadLock)} - {metadata}",
                    context);
        }
        ...
        public void UpdateLastAccessTime(Hash128 hash, LoggingContext context) {
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(HTTPCacheDatabase), $"{nameof(UpdateLastAccessTime)}({hash})",
                    context);
