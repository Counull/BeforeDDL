...
namespace Best.HTTP.Request.Authenticators {
    ...
    public class CredentialAuthenticator : IAuthenticator {
        ...
        public void SetupRequest(HTTPRequest request) {
            HTTPManager.Logger.Information(nameof(CredentialAuthenticator),
                $"SetupRequest({request}, {Credentials?.Type})", request.Context);
            ...
            switch (Credentials.Type) {
                ...
                case AuthenticationTypes.Digest:
                    ...
                    if (digest != null) {
                        string authentication = digest.GenerateResponseHeader(Credentials, false, request.MethodType,
                            request.CurrentUri);
        ...
        public bool HandleChallange(HTTPRequest req, HTTPResponse resp) {
            ...
            HTTPManager.Logger.Information(nameof(CredentialAuthenticator),
                $"HandleChallange({req}, {resp}, \"{www_authenticate}\")", req.Context);
            ...
            if (!string.IsNullOrEmpty(authHeader)) {
                ...
                if (this.Credentials != null && digest.IsUriProtected(req.CurrentUri) &&
                    (!req.HasHeader("Authorization") || digest.Stale))
                    return true;
            }
