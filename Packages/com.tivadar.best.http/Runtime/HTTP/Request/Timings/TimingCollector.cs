...
namespace Best.HTTP.Request.Timings {
    ...
    public sealed class TimingCollector {
        ...
        public void StartNext(string eventName) =>
            TimingEventHelper.Enqueue(new TimingEventInfo(this.ParentRequest, TimingEvents.StartNext, eventName));

        /// <summary>
        /// Finish the last event.
        /// </summary>
        public void Finish() =>
            TimingEventHelper.Enqueue(new TimingEventInfo(this.ParentRequest, TimingEvents.Finish, null));
            ...
            switch (timingEvent.Event) {
                case TimingEvents.StartNext:
                    if (this._partialEvent.IsSet()) {
                        ...
                        AddEvent(this._partialEvent.EventName, this._partialEvent.StartedAt,
                            timingEvent.Time - this._partialEvent.StartedAt);
                    }
