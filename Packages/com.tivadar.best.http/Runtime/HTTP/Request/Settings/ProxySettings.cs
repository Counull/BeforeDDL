...
namespace Best.HTTP.Request.Settings {
    ...
    public class ProxySettings {
        ...
        public void SetupRequest(HTTPRequest request) {
            ...
            if (!HTTPProtocolFactory.IsSecureProtocol(currentUri) && this.HasProxyFor(currentUri) &&
                !request.HasHeader("Proxy-Connection"))
                request.AddHeader("Proxy-Connection", tryToKeepAlive ? "Keep-Alive" : "Close");

            // Proxy Authentication
            if (!HTTPProtocolFactory.IsSecureProtocol(currentUri) && HasProxyFor(currentUri) &&
                ...
                switch (Proxy.Credentials.Type) {
                    case AuthenticationTypes.Basic:
                        ...
                        var token = Convert.ToBase64String(Encoding.UTF8.GetBytes(this.Proxy.Credentials.UserName +
                            ":" + this.Proxy.Credentials.Password));
                    ...
                    case AuthenticationTypes.Digest:
                        ...
                        if (digest != null) {
                            string authentication = digest.GenerateResponseHeader(this.Proxy.Credentials, false,
                                request.MethodType, currentUri);
