...
namespace Best.HTTP.Request.Authentication {
    ...
    public sealed class Digest {
        ...
        /// </summary>
        public string GenerateResponseHeader(Credentials credentials, bool isProxy, HTTPMethods methodType,
            Uri currentUri) {
            try {
                switch (Type) {
                    ...
                    case AuthenticationTypes.Digest: {
                        ...
                        string cnonce = new System.Random(this.GetHashCode()).Next(int.MinValue, int.MaxValue)
                            .ToString("X8");
                        ...
                        switch (Algorithm.TrimAndLower()) {
                            ...
                            case "md5-sess":
                                if (string.IsNullOrEmpty(this.HA1Sess))
                                    this.HA1Sess = string.Format("{0}:{1}:{2}:{3}:{4}", credentials.UserName, Realm,
                        ...
                        string qop = this.QualityOfProtections != null
                            ? this.QualityOfProtections.TrimAndLower()
                            : null;
                        ...
                        if (qop == null) {
                            string HA2 = string.Concat(HTTPRequest.MethodNames[(byte) methodType], ":",
                                currentUri.GetRequestPathAndQueryURL()).CalculateMD5Hash();
