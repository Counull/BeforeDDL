...
namespace Best.HTTP.Profiler.Network {
    public static class NetworkStatsCollector {
        ...
        public static int BufferedToSend {
            get => _bufferedToSend;
        }

        private static int _bufferedToSend;

        public static int ReceivedAndUnprocessed {
            get => _receivedAndUnprocessed;
        }
        ...
        internal static void IncrementTotalNetworkBytesReceived(int amount) =>
            Interlocked.Add(ref _totalNetworkBytesReceived, amount);

        internal static void IncrementTotalNetworkBytesSent(int amount) =>
            Interlocked.Add(ref _totalNetworkBytesSent, amount);

        internal static void IncrementBufferedToSend(int amount) => Interlocked.Add(ref _bufferedToSend, amount);

        internal static void IncrementReceivedAndUnprocessed(int amount) =>
            Interlocked.Add(ref _receivedAndUnprocessed, amount);
