...
namespace Best.HTTP.Shared.Databases {
    ...
    public sealed class DiskManager<T> : IDisposable {
        ...
        struct CachePointer<CacheType> {
            public static readonly CachePointer<CacheType> Empty = new CachePointer<CacheType>
                {Position = -1, Length = -1, Content = default(CacheType)};
        ...
        public DiskManager(Stream stream, Stream freeListStream, IDiskContentParser<T> contentParser,
        ...
        public BufferSegment CalculateHash() {
            using (new WriteLock(this.rwLock)) {
                ...
                var digest =
                    Best.HTTP.SecureProtocol.Org.BouncyCastle.Security.DigestUtilities.GetDigest(
                        this.options.HashDigest);
