...
namespace Best.HTTP.Shared.Databases {
    ...
    public abstract class MetadataService<MetadataType, ContentType> where MetadataType : Metadata, new() {
        ...
        protected MetadataService(IndexingService<ContentType, MetadataType> indexingService,
            IEmptyMetadataIndexFinder<MetadataType> emptyMetadataIndexFinder) {
        ...
        /// </summary>
        protected MetadataType CreateDefault(ContentType content, int filePos, int length,
            Action<ContentType, MetadataType> setupCallback) {
            var metadata = new MetadataType();
