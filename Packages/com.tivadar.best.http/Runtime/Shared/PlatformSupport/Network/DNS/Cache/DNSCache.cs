...
namespace Best.HTTP.Shared.PlatformSupport.Network.DNS.Cache {
    ...
    public readonly struct DNSQueryResult {
        ...
        public override string ToString() {
            if (this.Error != null)
                return
                    $"[{nameof(DNSQueryResult)}(\"{HostName}\", {this.Addresses?.Length}, Error: \"{this.Error?.Message}\")]";
            else
    ...
    public sealed class DNSIPAddress {
        ...
        public override string ToString() =>
            $"[{nameof(DNSIPAddress)}({this.IPAddress}, Working: {this.IsWorkedLastTime})]";
    }
    ...
    public static class DNSCache {
        ...
        private static ConcurrentDictionary<Hash128, DNSCacheEntry> _cache =
            new ConcurrentDictionary<Hash128, DNSCacheEntry>();
        ...
        public static void ReportAsNonWorking(string hostName, IPAddress address, LoggingContext context) {
            ...
            if (_cache.TryGetValue(key, out var entry)) {
                HTTPManager.Logger.Verbose(nameof(DNSCache),
                    $"{nameof(ReportAsNonWorking)}(\"{hostName}\", {address}) - CacheEntry found", context);
                entry.ReportNonWorking(address, context);
            }
            else
                HTTPManager.Logger.Verbose(nameof(DNSCache),
                    $"{nameof(ReportAsNonWorking)}(\"{hostName}\", {address}) - CacheEntry not found!", context);
        }
        ...
        public static void Query(DNSQueryParameters parameters) {
            ...
            if (IPAddress.TryParse(parameters.Hostname, out var ip) &&
                ...
                try {
                    if (HTTPManager.Logger.IsDiagnostic)
                        HTTPManager.Logger.Verbose(nameof(DNSCache),
                            $"{nameof(Query)}(\"{parameters.Hostname}\") - It's already an IP address, skipping DNS query...",
                            parameters.Context);

                    parameters.Callback?.Invoke(parameters,
                        new DNSQueryResult(parameters.Hostname, new DNSIPAddress[] {new DNSIPAddress(ip)}, null));
                }
                catch (Exception ex) {
                    HTTPManager.Logger.Exception(nameof(DNSCache), $"{nameof(Query)} - Callback", ex,
                        parameters.Context);
                }
            ...
            if (!parameters.IsPrefetch && _cache.TryGetValue(parameters.Key, out var entry)) {
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(DNSCache),
                        $"{nameof(Query)}(\"{parameters.Hostname}\", \"{parameters.Key}\") - Cache hit: {entry}",
                        parameters.Context);
                ...
                if (addresses != null && addresses.Length > 0) {
                    ...
                    try {
                        ...
                        HTTPManager.Logger.Exception(nameof(DNSCache),
                            $"{nameof(Query)}(\"{parameters.Hostname}\", \"{parameters.Key}\") - Cache hit - QueryImpl",
                            ex, parameters.Context);
                    }
                    ...
                    HTTPManager.Logger.Verbose(nameof(DNSCache),
                        $"{nameof(Query)}(\"{parameters.Hostname}\", \"{parameters.Key}\") - CacheEntry found, but no addresses returned!",
                        parameters.Context);
            }
            ...
            if (!parameters.IsPrefetch) {
                ...
                if (!ar.CompletedSynchronously && parameters.Token != CancellationToken.None)
                    Extensions.Timer.Add(new TimerData(Options.CancellationCheckGranularity, /*query*/ parameters,
                        CheckForCanceled));
            }

            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(DNSCache),
                    $"{nameof(Query)}(\"{parameters.Hostname}\", \"{parameters.Key}\") - Cache miss/Prefetch! {nameof(Dns.BeginGetHostAddresses)} called!",
                    parameters.Context);
        }
        ...
        private static void OnGetHostAddresses(IAsyncResult ar) {
            ...
            callback = Interlocked.CompareExchange<Action<DNSQueryParameters, DNSQueryResult>>(ref parameters.Callback,
                null, callback);
            ...
            try {
                ...
                if (HTTPManager.Logger.IsDiagnostic) {
                    ...
                    HTTPManager.Logger.Verbose(nameof(DNSCache),
                        $"{nameof(OnGetHostAddresses)}({parameters}) - {nameof(Dns.EndGetHostAddresses)} returned with {addresses?.Length} IPs: {ips}",
                        parameters.Context);
                }
                ...
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(DNSCache),
                        $"{nameof(OnGetHostAddresses)}({parameters}) - entry added to/updated in the cache: {entry}. Cache size: {_cache.Count}",
                        parameters.Context);
                ...
                if (entry != null && Interlocked.CompareExchange(ref _isMaintenanceScheduled, 1, 0) == 0) {
                    ...
                    if (HTTPManager.Logger.IsDiagnostic)
                        HTTPManager.Logger.Verbose(nameof(DNSCache),
                            $"{nameof(OnGetHostAddresses)}({parameters}) - Scheduled maintenance rutin",
                            parameters.Context);
                }
                ...
                try {
                    ...
                    HTTPManager.Logger.Exception(nameof(DNSCache),
                        $"{nameof(OnGetHostAddresses)}({parameters}) - callback", ex, parameters.Context);
                }
            }
            catch (Exception ex) {
                ...
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(DNSCache),
                        $"{nameof(OnGetHostAddresses)}({parameters}) calling callback with no addresses!",
                        parameters.Context);
                ...
                try {
                    ...
                    HTTPManager.Logger.Exception(nameof(DNSCache),
                        $"{nameof(OnGetHostAddresses)}({parameters}) - callback", e, parameters.Context);
                }
            }
        }
        ...
        private static bool CheckForCanceled(DateTime now, object context) {
            ...
            if (query.Token.IsCancellationRequested) {
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(DNSCache),
                        $"{nameof(CheckForCanceled)}({query}) - Token.IsCancellationRequested!", query.Context);

                var callback = query.Callback;

                callback = Interlocked.CompareExchange<Action<DNSQueryParameters, DNSQueryResult>>(ref query.Callback,
                    null, callback);

                try {
                    callback?.Invoke(query,
                        new DNSQueryResult(query.Hostname, null, new TimeoutException("DNS Query Timed Out")));
                }
                catch (Exception ex) {
                    HTTPManager.Logger.Exception(nameof(DNSCache), $"{nameof(CheckForCanceled)}({query}) - callback",
                        ex, query.Context);
                }
        ...
        private static bool Maintenance(DateTime now, object context) {
            ...
            foreach (var kvp in _cache) {
                ...
                if (entry.IsReadyToRemove(now)) {
                    if (_cache.TryRemove(key, out _)) {
                        if (HTTPManager.Logger.IsDiagnostic)
                            HTTPManager.Logger.Verbose(nameof(DNSCache),
                                $"{nameof(Maintenance)}.{nameof(DNSCacheEntry.IsReadyToRemove)}: Removed entry from cache: {entry}");
                    }
                    else if (HTTPManager.Logger.IsDiagnostic)
                        HTTPManager.Logger.Verbose(nameof(DNSCache),
                            $"{nameof(Maintenance)}.{nameof(DNSCacheEntry.IsReadyToRemove)}: Couldn't remove entry from cache: {entry}");
                }
                else if (entry.IsStalled(now)) {
                    if (HTTPManager.Logger.IsDiagnostic)
                        HTTPManager.Logger.Verbose(nameof(DNSCache),
                            $"{nameof(Maintenance)}.{nameof(DNSCacheEntry.IsStalled)}: Refreshing entry: {entry}");
            ...
            bool removeShedule =
                _cache.Count == 0 && Interlocked.CompareExchange(ref _isMaintenanceScheduled, 0, 1) == 1;
            ...
            return
                !removeShedule; // Timer expects false to remove the timer, so here we actually have to negate removeSchedule to remove.
        }
    }
}
#endif