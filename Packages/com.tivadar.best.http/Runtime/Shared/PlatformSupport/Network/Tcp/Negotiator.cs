...
namespace Best.HTTP.Shared.PlatformSupport.Network.Tcp {
    ...
    public interface INegotiationPeer {
        ...
        bool MustStopAdvancingToNextStep(Negotiator negotiator, NegotiationSteps finishedStep,
            NegotiationSteps nextStep, Exception error);
        ...
        void OnNegotiationFinished(Negotiator negotiator, PeekableContentProviderStream stream, TCPStreamer streamer,
    ...
    public class Negotiator {
        ...
        /// </summary>
        public NegotiationParameters Parameters {
        ...
        public void Start() {
            ...
            if (!this._peer.MustStopAdvancingToNextStep(this, NegotiationSteps.Start, NegotiationSteps.DNSQuery,
                    null)) {
                Uri target =
                    this._parameters.proxy != null &&
                    this._parameters.proxy.UseProxyForAddress(this._parameters.targetUri)
                        ? this._parameters.proxy.Address
                        : this._parameters.targetUri;
        ...
        public void OnCancellationRequested() {
            HTTPManager.Logger.Information(nameof(Negotiator), $"{nameof(OnCancellationRequested)}()",
        ...
        private void OnDNSCacheQueryFinished(DNSQueryParameters dnsParameters, DNSQueryResult result) {
            HTTPManager.Logger.Information(nameof(Negotiator),
                $"{nameof(OnDNSCacheQueryFinished)}({dnsParameters}, {result})", this._parameters.context);

            if (!this._peer.MustStopAdvancingToNextStep(this, NegotiationSteps.DNSQuery, NegotiationSteps.TCPRace,
                    result.Error)) {
        ...
        private void OnTCPRaceFinished(TCPRaceParameters parameters, TCPRaceResult raceResult) {
            HTTPManager.Logger.Information(nameof(Negotiator),
                $"{nameof(OnTCPRaceFinished)}({parameters}, {raceResult})", this._parameters.context);

            NegotiationSteps nextStep = this._parameters.proxy != null
                ? NegotiationSteps.Proxy
                : NegotiationSteps.TLSNegotiation;
        ...
        private void OnProxyNegotiated(HTTP.Proxies.ProxyConnectParameters parameters) {
            HTTPManager.Logger.Information(nameof(Negotiator), $"{nameof(OnProxyNegotiated)}({parameters})",
                this._parameters.context);

            NegotiationSteps nextStep = this._parameters.negotiateTLS
                ? NegotiationSteps.TLSNegotiation
                : NegotiationSteps.Finish;

            if (!this._peer.MustStopAdvancingToNextStep(this, NegotiationSteps.Proxy, nextStep, null))
                NegotiateTLS();
        }

        private void OnProxyNegotiationFailed(HTTP.Proxies.ProxyConnectParameters parameters, Exception error,
            bool resendForAuthentication) {
            HTTPManager.Logger.Information(nameof(Negotiator),
                $"{nameof(OnProxyNegotiationFailed)}({parameters}, {error}, {resendForAuthentication})",
                this._parameters.context);
        ...
        private void NegotiateTLS() {
            try {
                var hostSettings =
                    this.Parameters.hostSettings; //HTTPManager.PerHostSettings.Get(this._parameters.targetUri);

                if (this._parameters.negotiateTLS) {
                    HTTPManager.Logger.Information(nameof(Negotiator), $"{nameof(NegotiateTLS)}()",
                        this._parameters.context);
                    ...
                    switch (handlerType) {
                        ...
                        case TLSHandlers.BouncyCastle: {
                            ...
                            if (hostSettings.TLSSettings.BouncyCastleSettings.TlsClientFactory == null) {
                                tlsClient = BouncyCastleSettings.DefaultTlsClientFactory(this._parameters.targetUri,
                                    protocols, this._parameters.context);
                            }
                            else {
                                try {
                                    tlsClient = hostSettings.TLSSettings.BouncyCastleSettings.TlsClientFactory(
                                        this._parameters.targetUri, protocols, this._parameters.context);
                                }
                                catch (Exception ex) {
                                    HTTPManager.Logger.Exception(nameof(Negotiator),
                                        nameof(hostSettings.TLSSettings.BouncyCastleSettings.TlsClientFactory), ex,
                                        this._parameters.context);
                                }
                            ...
                            new NonblockingBCTLSStream(this._streamer, handler, tlsClient, true,
                                    this.Parameters.hostSettings.LowLevelConnectionSettings.ReadBufferSize)
                        ...
                        case TLSHandlers.Framework:
                            new FrameworkTLSStream(this._streamer, this._parameters.targetUri.Host,
                                    this.Parameters.hostSettings)
                    ...
                    HTTPManager.Logger.Information(nameof(Negotiator), $"{nameof(this._peer.OnNegotiationFinished)}()",
                        this._parameters.context);

                    if (this._stream is not NonblockingTCPStream)
                        this._stream = new NonblockingTCPStream(this._streamer, true,
                            this.Parameters.hostSettings.LowLevelConnectionSettings.ReadBufferSize);
                    this._peer.OnNegotiationFinished(this, this._stream, this._streamer, HTTPProtocolFactory.W3C_HTTP1);
                }
            }
            catch (Exception ex) {
                this._peer.MustStopAdvancingToNextStep(this, NegotiationSteps.TLSNegotiation, NegotiationSteps.Finish,
                    ex);
            }
        }

        private void OnFramework_TLSNegotiated(FrameworkTLSStream stream, TCPStreamer streamer, string alpn,
            Exception error) {
            HTTPManager.Logger.Information(nameof(Negotiator),
                $"{nameof(OnFramework_TLSNegotiated)}(\"{alpn}\", {error})", this._parameters.context);
...
#if !BESTHTTP_DISABLE_ALTERNATE_SSL
        private void OnBC_TLSNegotiated(NonblockingBCTLSStream stream, TCPStreamer streamer,
            AbstractTls13Client tlsClient, Exception error) {
            string alpn = tlsClient.GetNegotiatedApplicationProtocol();
            HTTPManager.Logger.Information(nameof(Negotiator), $"{nameof(OnBC_TLSNegotiated)}(\"{alpn}\", {error})",
                this._parameters.context);
...
#if UNITY_WINDOWS || UNITY_EDITOR
        private void SetKeepAlive(System.Net.Sockets.Socket socket, bool on, uint keepAliveTime,
