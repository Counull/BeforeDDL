...
namespace Best.HTTP.Shared.PlatformSupport.Network.Tcp {
    ...
    public sealed class TCPRingmasterSettings {
        ...
        public override string ToString() =>
            $"[{nameof(TCPRingmasterSettings)} {this.MaxSimultaneousRacers}, {this.ShuffleAddresses}, {this.CancellationCheckingGranularity}]";
    }
    ...
    sealed class Race {
        ...
        public override string ToString() =>
            $"[{nameof(Race)} {this.Parameters}, {this.NextAddressIndex}, {this.RunningLanes}]";
    }
    ...
    sealed class RacingLane {
        ...
        public override string ToString() =>
            $"[{nameof(RacingLane)} {this.Race}, {this.AddressIndex}, {this.LaneIndex}, {this.Socket}]";
    }
    ...
    public sealed class TCPRaceParameters {
        ...
        public override string ToString() {
            ...
            return
                $"[{nameof(TCPRaceParameters)} \"{this.Hostname}:{this.Port}\" Addresses({this.Addresses.Length}): {ips}]";
        }
    }
    ...
    public static class TCPRingmaster {
        ...
        public static void StartCompetion(TCPRaceParameters parameters) {
            ...
            Race race = new Race {
                ...
                NextAddressIndex = 0,
                RunningLanes = Math.Min(parameters.Addresses.Length,
                    options.TCPRingmasterSettings.MaxSimultaneousRacers),
            };
            ...
            if (algo != null) {
                try {
                    ...
                    HTTPManager.Logger.Exception(nameof(TCPRingmaster),
                        $"{nameof(options.TCPRingmasterSettings.CustomAddressShuffleAlgorithm)}({parameters})", ex,
                        parameters.Context);
                }
            }
            else {
            ...
            for (int i = 0; i < parameters.Addresses.Length - 1; i++) {
                ...
                if (!address.IsWorkedLastTime) {
                    Array.Copy(parameters.Addresses, i + 1, parameters.Addresses, i,
                        parameters.Addresses.Length - i - 1);
            ...
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(TCPRingmaster),
                    $"{nameof(StartCompetion)}({parameters}) - creating {race.RunningLanes} lane(s)",
                    parameters.Context);
            ...
            if (parameters.Token != CancellationToken.None)
                Extensions.Timer.Add(new Extensions.TimerData(
                    options.TCPRingmasterSettings.CancellationCheckingGranularity, race, CheckForCanceled));
        }
        ...
        private static void OnLaneFinished(IAsyncResult ar) {
            ...
            try {
                ...
                if (callback != null) {
                    ...
                    if (HTTPManager.Logger.IsDiagnostic)
                        HTTPManager.Logger.Verbose(nameof(TCPRingmaster),
                            $"{nameof(OnLaneFinished)}(Lane: {lane.LaneIndex}, Addr.: {lane.AddressIndex}) - Winner lane! Address: {lane.Race.Parameters.Addresses[lane.AddressIndex]}, remaining lanes: {runningLanes}",
                            lane.Race.Parameters.Context);
                    ...
                    try {
                        ...
                        HTTPManager.Logger.Exception(nameof(TCPRingmaster),
                            $"{nameof(OnLaneFinished)}({lane.LaneIndex}) - callback", ex, lane.Race.Parameters.Context);
                    }
                }
                else {
                    if (HTTPManager.Logger.IsDiagnostic)
                        HTTPManager.Logger.Verbose(nameof(TCPRingmaster),
                            $"{nameof(OnLaneFinished)}(Lane: {lane.LaneIndex}, Addr.: {lane.AddressIndex}) - Late lane, disconnecting...",
                            lane.Race.Parameters.Context);
                ...
                DNSCache.ReportAsNonWorking(lane.Race.Parameters.Hostname,
                    lane.Race.Parameters.Addresses[lane.AddressIndex].IPAddress, lane.Race.Parameters.Context);

                var nextIndex = Interlocked.Increment(ref lane.Race.NextAddressIndex);
                if (nextIndex < lane.Race.Parameters.Addresses.Length) {
                    if (HTTPManager.Logger.IsDiagnostic)
                        HTTPManager.Logger.Verbose(nameof(TCPRingmaster),
                            $"{nameof(OnLaneFinished)}(Lane: {lane.LaneIndex}, Addr.: {lane.AddressIndex}) - Couldn't connect to address, grabbing new index: {nextIndex}",
                            lane.Race.Parameters.Context);

                    lane.AddressIndex = nextIndex;
                    lane.Socket = new Socket(lane.Race.Parameters.Addresses[lane.AddressIndex].IPAddress.AddressFamily,
                        SocketType.Stream, ProtocolType.Tcp);
                    lane.Socket.BeginConnect(lane.Race.Parameters.Addresses[lane.AddressIndex].IPAddress,
                        lane.Race.Parameters.Port, OnLaneFinished, lane);
                }
                else {
                    ...
                    if (runningLanes == 0) {
                        ...
                        if (HTTPManager.Logger.IsDiagnostic)
                            HTTPManager.Logger.Verbose(nameof(TCPRingmaster),
                                $"{nameof(OnLaneFinished)}(Lane: {lane.LaneIndex}, Addr.: {lane.AddressIndex}) Race is over, calling callback if it's still available",
                                lane.Race.Parameters.Context);

                        var callback = lane.Race.Parameters.AnnounceWinnerCallback;
                        callback = Interlocked.CompareExchange(ref lane.Race.Parameters.AnnounceWinnerCallback, null,
                            callback);
                        ...
                        try {
                            ...
                            HTTPManager.Logger.Exception(nameof(TCPRingmaster),
                                $"{nameof(OnLaneFinished)}(Lane: {lane.LaneIndex}, Addr.: {lane.AddressIndex}) - callback",
                                nex, lane.Race.Parameters.Context);
                        }
                    }
                    else if (HTTPManager.Logger.IsDiagnostic)
                        HTTPManager.Logger.Verbose(nameof(TCPRingmaster),
                            $"{nameof(OnLaneFinished)}(Lane: {lane.LaneIndex}, Addr.: {lane.AddressIndex}) Run out of free racers, there are still running racers({runningLanes}), retiring this lane.",
                            lane.Race.Parameters.Context);
                }
            }
        }
        ...
        private static void OnSocketDisconnect(IAsyncResult ar) {
            ...
            try {
                ...
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Exception(nameof(TCPRingmaster),
                        $"{nameof(OnSocketDisconnect)}(Lane: {lane.LaneIndex}, Addr.: {lane.AddressIndex})", ex,
                        lane.Race.Parameters.Context);
            }
            finally {
                ...
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(TCPRingmaster),
                        $"{nameof(OnSocketDisconnect)}(Lane: {lane.LaneIndex}, Addr.: {lane.AddressIndex})",
                        lane.Race.Parameters.Context);
            }
        }
        ...
        private static bool CheckForCanceled(DateTime now, object context) {
            ...
            if (query.Parameters.Token.IsCancellationRequested) {
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(TCPRingmaster),
                        $"{nameof(CheckForCanceled)}({query}) - Token.IsCancellationRequested!",
                        query.Parameters.Context);

                var callback = query.Parameters.AnnounceWinnerCallback;

                callback = Interlocked.CompareExchange<Action<TCPRaceParameters, TCPRaceResult>>(
                    ref query.Parameters.AnnounceWinnerCallback, null, callback);

                try {
                    callback?.Invoke(query.Parameters,
                        new TCPRaceResult(null, new TimeoutException("DNS Query Timed Out")));
                }
                catch (Exception ex) {
                    HTTPManager.Logger.Exception(nameof(TCPRingmaster),
                        $"{nameof(CheckForCanceled)}({query}) - callback", ex, query.Parameters.Context);
                }
            }
