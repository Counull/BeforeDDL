...
namespace Best.HTTP.Shared.PlatformSupport.Network.Tcp {
    ...
    public sealed class TCPStreamer : IDisposable, IHeartbeat {
        ...
        /// </summary>
        public ITCPStreamerContentConsumer ContentConsumer {
            get => this._contentConsumer;
            set { this._contentConsumer = value; }
        }
        ...
        public bool IsConnectionClosed {
            get => this._disposed || this._closed == 1 || this._closeInitiatedByServer;
        }
        ...
        /// </summary>
        public int MinReceiveBufferSize {
            get => this.readState.minReceiveBufferSize;
        }
        ...
        /// </summary>
        public long Length {
            get => this.readState.bufferedLength;
        }
        ...
        public TCPStreamer(Socket socket, uint maxReadBufferSize, uint maxWriteBufferSize,
            LoggingContext _parentLoggingContext) {
            this._socket = socket;
            ...
            HTTPManager.Logger.Verbose(nameof(TCPStreamer),
                $"Created with minReceiveBufferSize: ({this.readState.minReceiveBufferSize:N0})", this._loggingContext);
        ...
        public void BeginReceive() {
            ...
            if (!this.IsConnectionClosed && length < MaxBufferedReadAmount &&
                (receiving = Interlocked.CompareExchange(ref this.readState.isReceiving, 1, 0)) == 0) {
#if LOG_TCP_STREAMER
                ...
                try {
                    ...
                    if (HTTPManager.Logger.IsDiagnostic)
                        HTTPManager.Logger.Exception(nameof(TCPStreamer), $"{nameof(this._socket.BeginReceive)}", e,
            ...
            try {
                ...
                if (!isClosed) {
                    ...
                    try {
                        ...
                        HTTPManager.Logger.Exception(nameof(TCPStreamer), "ContentConsumer.OnContent", e,
                            this._loggingContext);
                    }
                }
            }
                ...
                if (!Volatile.Read(ref this._disposed))
                    HTTPManager.Logger.Exception(nameof(TCPStreamer), $"{nameof(OnReceived)}({errorCode})", ex,
                        this._loggingContext);
            }
            finally {
                if (!isClosed) {
                    ...
                    try {
                        ...
                        HTTPManager.Logger.Exception(nameof(TCPStreamer), "ContentConsumer.OnConnectionClosed", e,
                            this._loggingContext);
                    }
        ...
        private bool SendFromQueue() {
            ...
            if (this.writeState._segmentsToWrite.TryDequeue(out var writeBuffer)) {
                ...
                socket.BeginSend(writeBuffer.Data, writeBuffer.Offset, writeBuffer.Count, SocketFlags.None,
                    OnWroteToNetwork, writeBuffer);
        ...
        private void OnWroteToNetwork(IAsyncResult ar) {
            ...
            try {
                ...
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Exception(nameof(TCPStreamer), $"{nameof(OnWroteToNetwork)}({ex.Message})", ex,
                        this._loggingContext);
            }
        }
        ...
        void IHeartbeat.OnHeartbeatUpdate(DateTime now, TimeSpan dif) {
            if (this.writeState._segmentsToWrite.Count > 0 &&
                Interlocked.CompareExchange(ref this.writeState._writeInProgress, 1, 0) == 0) {
                if (!SendFromQueue())
                    Interlocked.Exchange(ref this.writeState._writeInProgress, 0);
            }
        }
        ...
        internal void Close() {
            HTTPManager.Logger.Verbose(nameof(TCPStreamer),
                $"{nameof(Close)}({this._closed}, {this._socket?.Connected})", this._loggingContext);
