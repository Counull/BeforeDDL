...
namespace Best.HTTP.Shared.PlatformSupport.Network.Tcp.Streams {
    public sealed class FrameworkTLSByteForwarder : Stream, ITCPStreamerContentConsumer {
        ...
        public FrameworkTLSByteForwarder(TCPStreamer streamer, ITCPStreamerContentConsumer contentConsumer,
            long maxBufferSize, LoggingContext context) {
        ...
        void PullContentFromStreamer() {
            ...
            try {
                while (this._streamer.Length > 0 && this._length < this.MaxBufferSize) {
                    ...
                    if (tmp.Count <= 0) {
                        HTTPManager.Logger.Verbose(nameof(FrameworkTLSByteForwarder), $"DequeueReceived({tmp}) !",
                            this._context);
        ...
        public void /*ITCPStreamerContentConsumer.*/ OnContent(TCPStreamer streamer) {
            HTTPManager.Logger.Verbose(nameof(FrameworkTLSByteForwarder), $"OnContent({streamer?.Length})",
                this._context);
        ...
        public void /*ITCPStreamerContentConsumer.*/ OnConnectionClosed(TCPStreamer streamer) =>
            this._contentConsumer?.OnConnectionClosed(streamer);

        public void /*ITCPStreamerContentConsumer.*/ OnError(TCPStreamer streamer, Exception ex) =>
            this._contentConsumer?.OnError(streamer, ex);
        ...
        public override int Read(byte[] buffer, int offset, int count) {
            ...
            while (this._currentReadSegment == BufferSegment.Empty && this._segmentsToReadFrom.Count == 0) {
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(FrameworkTLSByteForwarder), $"WaitOne() for new data!",
                        this._context);
                ...
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(FrameworkTLSByteForwarder), $"WaitOne() returned!",
                        this._context);
            }
            ...
            while ((this._currentReadSegment != BufferSegment.Empty || this._segmentsToReadFrom.Count > 0) &&
                ...
                if (this._currentReadSegment != BufferSegment.Empty) {
                    ...
                    Array.Copy(this._currentReadSegment.Data, this._currentReadSegment.Offset, buffer, offset,
                        readCount);
                    offset += readCount;
                    count -= readCount;
                    ...
                    if (this._currentReadSegment.Count <= readCount)
                        ...
                        this._currentReadSegment =
                            this._currentReadSegment.Slice(this._currentReadSegment.Offset + readCount);
                }
                else {
            ...
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(FrameworkTLSByteForwarder),
                    $"Read() returns with readCount: {sumReadCount}, remaining: {this._length}", this._context);
        ...
        public override void Write(byte[] buffer, int offset, int count) {
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(FrameworkTLSByteForwarder),
                    $"Write({buffer.AsBuffer(offset, count)})", this._context);
