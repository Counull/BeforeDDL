...
namespace Best.HTTP.Shared.PlatformSupport.Network.Tcp.Streams {
    ...
    public sealed class FrameworkTLSStream : PeekableContentProviderStream, ITCPStreamerContentConsumer {
        public Action<FrameworkTLSStream, TCPStreamer, string /*negotiated appplication protocol*/, Exception>
            OnNegotiated;

        public long MaxBufferSize {
            get => Volatile.Read(ref this._maxBufferSize);
            set => Interlocked.Exchange(ref this._maxBufferSize, value);
        }
        ...
        private bool OnUserCertificationValidation(object sender, X509Certificate certificate, X509Chain chain,
            SslPolicyErrors sslPolicyErrors) {
            HTTPManager.Logger.Information(nameof(FrameworkTLSStream),
                $"{nameof(OnUserCertificationValidation)}({sender}, {certificate}, {chain}, {sslPolicyErrors})",
                this._context);

            var validator = this._hostSettings.TLSSettings.FrameworkTLSSettings.CertificationValidator;
            if (validator == null)
                return FrameworkTLSSettings.DefaultCertificationValidator(_targetHost, certificate, chain,
                    sslPolicyErrors);

            return validator(this._targetHost, certificate, chain, sslPolicyErrors);
        }

        private X509Certificate OnUserCertificationSelection(object sender, string targetHost,
            X509CertificateCollection localCertificates, X509Certificate remoteCertificate,
            string[] acceptableIssuers) {
            HTTPManager.Logger.Information(nameof(FrameworkTLSStream),
                $"{nameof(OnUserCertificationSelection)}({sender}, {targetHost}, {localCertificates}, {remoteCertificate}, {acceptableIssuers?.Length})",
                this._context);

            return this._hostSettings.TLSSettings.FrameworkTLSSettings.ClientCertificationProvider?.Invoke(targetHost,
                localCertificates, remoteCertificate, acceptableIssuers);
        }

        private void OnAuthenticatedAsClient(IAsyncResult ar) {
            HTTPManager.Logger.Information(nameof(FrameworkTLSStream), $"{nameof(OnAuthenticatedAsClient)}()",
                this._context);
            ...
            try {
                ...
                try {
                    ...
                    if (HTTPManager.Logger.IsDiagnostic)
                        HTTPManager.Logger.Exception(nameof(FrameworkTLSStream),
                            $"{nameof(OnAuthenticatedAsClient)}() - NegotiatedApplicationProtocol", ex, this._context);

                    if (!loggedWarning) {
                        loggedWarning = true;
                        HTTPManager.Logger.Warning(nameof(FrameworkTLSStream),
                            $"{nameof(OnAuthenticatedAsClient)}(): SslStream's NegotiatedApplicationProtocol inaccessible! Using http/1.",
                            this._context);
                    }
                }
#endif
                ...
                HTTPManager.Logger.Exception(nameof(FrameworkTLSStream), $"{nameof(OnAuthenticatedAsClient)}()", ex,
                    this._context);
        ...
        bool CallOnNegotiated(string alpn, Exception error) {
            HTTPManager.Logger.Verbose(nameof(FrameworkTLSStream), $"CallOnNegotiated(\"{alpn}\", {error})",
                this._context);
            ...
            if (callback != null) {
                try {
                    ...
                    HTTPManager.Logger.Exception(nameof(FrameworkTLSStream), "OnContent - OnNegotiated", ex,
                        this._streamer.Context);
                }
            }
            ...
            this._sslStream.ReadAsync(buffer, 0, buffer.Length)
                ...
                .ContinueWith((ti) => {
                    ...
                    try {
                        ...
                        if (HTTPManager.Logger.IsDiagnostic)
                            HTTPManager.Logger.Verbose(nameof(FrameworkTLSStream),
                                $"{nameof(OnRead)}({readCount}, {this.Length})", this._context);
        ...
        private void OnRead(IAsyncResult ar) {
            try {
                ...
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Verbose(nameof(FrameworkTLSStream),
                        $"{nameof(OnRead)}({readCount}, {ar.CompletedSynchronously})", this._context);
        ...
        public override int Read(byte[] buffer, int offset, int count) {
            lock (locker) return base.Read(buffer, offset, count);
        }

        public override void Write(byte[] buffer, int offset, int count) =>
            this._sslStream.Write(buffer, offset, count);
