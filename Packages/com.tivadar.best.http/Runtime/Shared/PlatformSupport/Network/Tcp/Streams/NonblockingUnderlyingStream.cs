...
namespace Best.HTTP.Shared.PlatformSupport.Network.Tcp.Streams {
    public sealed class NonblockingUnderlyingStream : PeekableContentProviderStream {
        ...
        public void BeginReceive() {
            if (base._length < this._maxBufferSize && Interlocked.CompareExchange(ref this._receiving, 1, 0) == 0 &&
                ...
                try {
                    ...
                    if (ar.CompletedSynchronously)
                        HTTPManager.Logger.Warning(nameof(NonblockingUnderlyingStream), $"CompletedSynchronously!",
                            this._context);
                }
                catch (Exception ex) {
                    if (HTTPManager.Logger.IsDiagnostic)
                        HTTPManager.Logger.Exception(nameof(NonblockingUnderlyingStream),
                            $"{nameof(this._stream.BeginRead)}", ex, this._context);
            ...
            try {
                ...
                if (!isClosed) {
                    ...
                    try {
                        ...
                        HTTPManager.Logger.Exception(nameof(NonblockingUnderlyingStream), "ContentConsumer.OnContent",
                            e, this._context);
                    }
                }
            }
                ...
                if (HTTPManager.Logger.IsDiagnostic)
                    HTTPManager.Logger.Exception(nameof(NonblockingUnderlyingStream), $"{nameof(OnReceived)}", ex,
                        this._context);
            }
            finally {
                if (!isClosed) {
                    ...
                    try {
                        ...
                        HTTPManager.Logger.Exception(nameof(NonblockingUnderlyingStream), "Consumer.OnConnectionClosed",
                            e, this._context);
                    }
                }
            }
