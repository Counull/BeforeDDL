...
namespace Best.HTTP.Shared.TLS {
    ...
    internal static class KeyLogFileWriter {
        private static string GetKeylogFileName() =>
        ...
        public static void WriteLabel(Labels label, byte[] clientRandom, TlsSecret secret) {
            if (clientRandom != null && secret != null) {
                ...
                if (!string.IsNullOrEmpty(SSLKEYLOGFILE))
                    using (var writer = new StreamWriter(System.IO.File.Open(SSLKEYLOGFILE, FileMode.Append,
                               FileAccess.Write, FileShare.ReadWrite)))
                        writer.WriteLine(
                            $"{label} {Hex.ToHexString(clientRandom)} {Hex.ToHexString((secret as AbstractTlsSecret).CopyData())}");
            }
        }
