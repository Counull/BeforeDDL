...
namespace Best.HTTP.Shared.TLS.Crypto.Impl {
    ...
    public unsafe static class FastChaCha7539EngineHelper {
        ...
        public static void ProcessBlocks2(ReadOnlySpan<byte> input, Span<byte> output, uint[] state, int rounds,
            byte[] keyStream) {
            fixed (byte* pinput = input)
            fixed (byte* poutput = output)
            fixed (uint* pstate = state)
            fixed (byte* pkeyStream = keyStream)
                ProcessBlocks2Impl(pinput, input.Length, poutput, output.Length, pstate, state.Length, rounds,
                    pkeyStream);
        }

        [BurstCompile]
        private static void ProcessBlocks2Impl([NoAlias] byte* input, int inputLen, [NoAlias] byte* output, int outLen,
            [NoAlias] uint* state, int stateLen, int rounds, [NoAlias] byte* keyStream) {
            if (Avx2.IsAvx2Supported) {
                ...
                n0 = Avx2.mm256_xor_si256(n0,
                    new v256(uInput[0], uInput[1], uInput[2], uInput[3])); // Load256_Byte(input)
                n1 = Avx2.mm256_xor_si256(n1,
                    new v256(uInput[4], uInput[5], uInput[6], uInput[7])); // Load256_Byte(input[0x20..])
                n2 = Avx2.mm256_xor_si256(n2,
                    new v256(uInput[8], uInput[9], uInput[10], uInput[11])); // Load256_Byte(input[0x40..])
                n3 = Avx2.mm256_xor_si256(n3,
                    new v256(uInput[12], uInput[13], uInput[14], uInput[15])); // Load256_Byte(input[0x60..])

                ulong* uOutput = (ulong*) output;
                uOutput[0] = n0.ULong0;
                uOutput[1] = n0.ULong1;
                uOutput[2] = n0.ULong2;
                uOutput[3] = n0.ULong3; //Store256_Byte(n0, output);
                uOutput[4] = n1.ULong0;
                uOutput[5] = n1.ULong1;
                uOutput[6] = n1.ULong2;
                uOutput[7] = n1.ULong3; //Store256_Byte(n1, output[0x20..]);
                uOutput[8] = n2.ULong0;
                uOutput[9] = n2.ULong1;
                uOutput[10] = n2.ULong2;
                uOutput[11] = n2.ULong3; //Store256_Byte(n2, output[0x40..]);
                uOutput[12] = n3.ULong0;
                uOutput[13] = n3.ULong1;
                uOutput[14] = n3.ULong2;
                uOutput[15] = n3.ULong3; //Store256_Byte(n3, output[0x60..]);
            }
#if !UNITY_ANDROID && !UNITY_IOS
            else if (Sse2.IsSse2Supported)
            {
                var x0 =
 Sse2.loadu_si128(state); //new v128(state[0], state[1], state[2], state[3]); //Load128_UInt32(state.AsSpan());
                var x1 =
 Sse2.loadu_si128(state + 4); //new v128(state[4], state[5], state[6], state[7]); //Load128_UInt32(state.AsSpan(4));
                var x2 =
 Sse2.loadu_si128(state + 8); //new v128(state[8], state[9], state[10], state[11]); //Load128_UInt32(state.AsSpan(8));
                var x3 =
 Sse2.loadu_si128(state + 12); //new v128(state[12], state[13], state[14], state[15]); //Load128_UInt32(state.AsSpan(12));
                ++state[12];
                ...
                var x0 = Neon
                    .vld1q_u32(state); //new v128(state[0], state[1], state[2], state[3]); //Load128_UInt32(state.AsSpan());
                var x1 = Neon.vld1q_u32(state +
                                        4); //new v128(state[4], state[5], state[6], state[7]); //Load128_UInt32(state.AsSpan(4));
                var x2 = Neon.vld1q_u32(state +
                                        8); //new v128(state[8], state[9], state[10], state[11]); //Load128_UInt32(state.AsSpan(8));
