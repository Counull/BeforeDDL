...
namespace Best.HTTP.Shared.TLS.Crypto.Impl {
    ...
    internal sealed unsafe class FastGcmBlockCipherHelper {
        ...
        public static unsafe void DecryptBlock(ReadOnlySpan<byte> input, Span<byte> output, Span<byte> ctrBlock,
            Span<byte> S, int BlockSize) {
        ...
        private unsafe static void DecryptBlock_Impl([NoAlias] byte* pinput, int inLen, [NoAlias] byte* poutput,
            int outLen, [NoAlias] byte* pctrBlock, [NoAlias] byte* pS, int BlockSize) {
