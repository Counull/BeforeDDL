    ...
    public class JPakeParticipant {
        ...
        public virtual JPakeRound1Payload CreateRound1PayloadToSend() {
            ...
            BigInteger[] knowledgeProofForX1 =
                JPakeUtilities.CalculateZeroKnowledgeProof(p, q, g, gx1, x1, participantId, digest, random);
            BigInteger[] knowledgeProofForX2 =
                JPakeUtilities.CalculateZeroKnowledgeProof(p, q, g, gx2, x2, participantId, digest, random);
        ...
        public virtual void ValidateRound1PayloadReceived(JPakeRound1Payload round1PayloadReceived) {
            ...
            JPakeUtilities.ValidateZeroKnowledgeProof(p, q, g, gx3, knowledgeProofForX3,
                round1PayloadReceived.ParticipantId, digest);
            JPakeUtilities.ValidateZeroKnowledgeProof(p, q, g, gx4, knowledgeProofForX4,
                round1PayloadReceived.ParticipantId, digest);
        ...
        public virtual JPakeRound2Payload CreateRound2PayloadToSend() {
            ...
            if (this.state < STATE_ROUND_1_VALIDATED)
                throw new InvalidOperationException(
                    "Round 1 payload must be validated prior to creating round 2 payload for " + this.participantId);
            ...
            BigInteger[] knowledgeProofForX2s =
                JPakeUtilities.CalculateZeroKnowledgeProof(p, q, gA, A, x2s, participantId, digest, random);
        ...
        public virtual void ValidateRound2PayloadReceived(JPakeRound2Payload round2PayloadReceived) {
            ...
            if (this.state < STATE_ROUND_1_VALIDATED)
                throw new InvalidOperationException(
                    "Round 1 payload must be validated prior to validation round 2 payload for " + this.participantId);
            ...
            JPakeUtilities.ValidateZeroKnowledgeProof(p, q, gB, b, knowledgeProofForX4s,
                round2PayloadReceived.ParticipantId, digest);
        ...
        public virtual BigInteger CalculateKeyingMaterial() {
            ...
            if (this.state < STATE_ROUND_2_VALIDATED)
                throw new InvalidOperationException("Round 2 payload must be validated prior to creating key for " +
                                                    participantId);
        ...
        public virtual JPakeRound3Payload CreateRound3PayloadToSend(BigInteger keyingMaterial) {
            ...
            if (this.state < STATE_KEY_CALCULATED)
                throw new InvalidOperationException(
                    "Keying material must be calculated prior to creating round 3 payload for " + this.participantId);
        ...
        /// <param name="keyingMaterial">The keying material as returned from CalculateKeyingMaterial().</param> 
        public virtual void ValidateRound3PayloadReceived(JPakeRound3Payload round3PayloadReceived,
            BigInteger keyingMaterial) {
            if (this.state >= STATE_ROUND_3_VALIDATED)
                throw new InvalidOperationException("Validation already attempted for round 3 payload for " +
                                                    this.participantId);
            if (this.state < STATE_KEY_CALCULATED)
                throw new InvalidOperationException(
                    "Keying material must be calculated prior to validating round 3 payload for " + this.participantId);
