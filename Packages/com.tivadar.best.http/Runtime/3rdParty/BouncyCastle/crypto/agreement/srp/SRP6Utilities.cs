...
namespace Best.HTTP.SecureProtocol.Org.BouncyCastle.Crypto.Agreement.Srp {
    public class Srp6Utilities {
        ...
        public static BigInteger CalculateX(IDigest digest, BigInteger N, byte[] salt, byte[] identity,
            byte[] password) {
        ...
        private static BigInteger HashPaddedTriplet(IDigest digest, BigInteger N, BigInteger n1, BigInteger n2,
            BigInteger n3) {
            int paddedLength = (N.BitLength + 7) / 8;
            int digestSize = digest.GetDigestSize();
