        ...
        internal int Generate(Span<byte> output, bool predictionResistant) {
            ...
            if (mR.Length == 8) // 64 bit block size
            {
                ...
                if (outputLen > BLOCK64_MAX_BITS_REQUEST / 8)
                    throw new ArgumentException("Number of bits per request limited to " + BLOCK64_MAX_BITS_REQUEST,
                        "output");
            }
            else {
                ...
                if (outputLen > BLOCK128_MAX_BITS_REQUEST / 8)
                    throw new ArgumentException("Number of bits per request limited to " + BLOCK128_MAX_BITS_REQUEST,
                        "output");
            }
