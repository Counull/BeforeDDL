    ...
    public sealed class Grain128AeadEngine
        ...
        private void InitGrain() {
            ...
            for (int quotient = 0; quotient < 8; ++quotient) {
                for (int remainder = 0; remainder < 8; ++remainder) {
                    uint outputZ = GetOutput();
                    nfsr = Shift(nfsr,
                        (GetOutputNFSR() ^ lfsr[0] ^ outputZ ^ (uint) ((workingKey[quotient]) >> remainder)) & 1);
                    lfsr = Shift(lfsr,
                        (GetOutputLFSR() ^ outputZ ^ (uint) ((workingKey[quotient + 8]) >> remainder)) & 1);
                }
            }
        ...
        private uint GetOutputNFSR() {
            ...
            return (b0 ^ b26 ^ b56 ^ b91 ^ b96 ^ b3 & b67 ^ b11 & b13 ^ b17 & b18
                    ^ b27 & b59 ^ b40 & b48 ^ b61 & b65 ^ b68 & b84 ^ b22 & b24 & b25 ^ b70 & b78 & b82 ^
                    b88 & b92 & b93 & b95) & 1;
        }
