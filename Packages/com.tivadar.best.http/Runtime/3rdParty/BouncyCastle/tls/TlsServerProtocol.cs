...
namespace Best.HTTP.SecureProtocol.Org.BouncyCastle.Tls {
    public class TlsServerProtocol
        ...
        protected virtual ServerHello GenerateServerHello(ClientHello clientHello,
            ...
            byte[] clientRenegExtData =
                TlsUtilities.GetExtensionData(m_clientExtensions, ExtensionType.renegotiation_info);
            ...
            if (m_serverExtensions.Count > 0) {
                ...
                if (!resumedSession) {
                    ...
                    else if (TlsUtilities.HasExpectedEmptyExtensionData(m_serverExtensions,
                        ...
                        case CS_START: {
                            ...
                            if (null != serverCredentials) {
                                ...
                                if (null == m_certificateRequest) {
                                    ...
                                    this.m_certificateRequest = TlsUtilities.ValidateCertificateRequest(
                                    ...
                                    if (ProtocolVersion.TLSv12.Equals(securityParameters.NegotiatedVersion)) {
                                        TlsUtilities.TrackHashAlgorithms(m_handshakeHash,
                                            securityParameters.ServerSigAlgs);

                                        if (m_tlsServerContext.Crypto.HasAnyStreamVerifiers(securityParameters
                                                .ServerSigAlgs)) {
                                            m_handshakeHash.ForceBuffering();
                                        }
                                    }
                                    else {
                                        if (m_tlsServerContext.Crypto.HasAnyStreamVerifiersLegacy(m_certificateRequest
                                                .CertificateTypes)) {
                                            m_handshakeHash.ForceBuffering();
                                        }
                                    }
