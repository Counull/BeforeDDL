...
namespace Best.HTTP.SecureProtocol.Org.BouncyCastle.Tls {
    public class TlsClientProtocol
                ...
                case HandshakeType.certificate: {
                    switch (m_connectionState) {
                        ...
                        case CS_SERVER_SUPPLEMENTAL_DATA: {
                             ...
                             */
                            m_authentication = TlsUtilities.ReceiveServerCertificate(m_tlsClientContext, m_tlsClient,
                                buf,
                                m_serverExtensions);
                            break;
                        }
                                    ...
                                    if (clientAuthCredentials is TlsCredentialedSigner) {
                                        ...
                                        if (ProtocolVersion.TLSv12.Equals(securityParameters.NegotiatedVersion)) {
                                            ...
                                            if (clientAuthStreamSigner == null) {
                                                TlsUtilities.TrackHashAlgorithmClient(m_handshakeHash,
                                                    clientAuthAlgorithm);
                                            }
                                        }
        ...
        protected override void Send13CertificateMessage(Certificate certificate) {
            ...
            certificate = new Certificate(this.m_certificateRequest.GetCertificateRequestContext(),
                certificate.GetCertificateEntryList());
...
//            }
            ...
            if (securityParameters.IsRenegotiating) {
                ...
                m_clientExtensions[ExtensionType.renegotiation_info] =
                    TlsProtocol.CreateRenegotiationInfo(saved.LocalVerifyData);
            }
            else {
                /*
                 * RFC 5746 3.4. Client Behavior: Initial Handshake (both full and session-resumption)
                ...
                if (noRenegExt && noRenegScsv) {
                    ...
                    offeredCipherSuites =
                        Arrays.Append(offeredCipherSuites, CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV);
                }
            }
