                ...
                foreach (Asn1Encodable item in reqInfo.Attributes) {
                    ...
                    if (attributePkcs.AttrType.Equals(PkcsObjectIdentifiers.Pkcs9AtExtensionRequest)) {
                        ...
                        foreach (Asn1Encodable seqItem in extensionSequence) {
                            ...
                            if (itemSeq.Count == 2) {
                                generator.AddExtension(DerObjectIdentifier.GetInstance(itemSeq[0]), false,
                                    Asn1OctetString.GetInstance(itemSeq[1]).GetOctets());
                            }
                            else if (itemSeq.Count == 3) {
                                generator.AddExtension(DerObjectIdentifier.GetInstance(itemSeq[0]),
                                    DerBoolean.GetInstance(itemSeq[1]).IsTrue,
                                    Asn1OctetString.GetInstance(itemSeq[2]).GetOctets());
                            }
                            else {
                                throw new ArgumentException("incorrect sequence size of X509Extension got " +
                                                            itemSeq.Count + " expected 2 or 3");
                            }
                        }
