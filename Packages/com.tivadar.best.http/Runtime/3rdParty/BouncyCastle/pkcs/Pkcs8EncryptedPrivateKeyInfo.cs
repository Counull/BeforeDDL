...
namespace Best.HTTP.SecureProtocol.Org.BouncyCastle.Pkcs {
    ...
    public class Pkcs8EncryptedPrivateKeyInfo {
        ...
        public PrivateKeyInfo DecryptPrivateKeyInfo(IDecryptorBuilderProvider inputDecryptorProvider) {
            try {
                ICipherBuilder decryptorBuilder =
                    inputDecryptorProvider.CreateDecryptorBuilder(encryptedPrivateKeyInfo.EncryptionAlgorithm);

                ICipher encIn =
                    decryptorBuilder.BuildCipher(new MemoryInputStream(encryptedPrivateKeyInfo.GetEncryptedData()));
