    ...
    class PemParser {
        ...
        internal Asn1Sequence ReadPemObject(
            ...
            while ((line = ReadLine(inStream)) != null) {
                if (Org.BouncyCastle.Utilities.Platform.StartsWith(line, _header1) ||
                    Org.BouncyCastle.Utilities.Platform.StartsWith(line, _header2)) {
                    break;
                }
            }

            while ((line = ReadLine(inStream)) != null) {
                if (Org.BouncyCastle.Utilities.Platform.StartsWith(line, _footer1) ||
                    Org.BouncyCastle.Utilities.Platform.StartsWith(line, _footer2)) {
                    break;
                }
