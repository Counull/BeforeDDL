...
namespace Cainos.CustomizablePixelCharacter {
    public class PixelCharacterController : MonoBehaviour {
        ...
        private static readonly float
            CLIMB_POS_LERP_SPEED = 7.5f; // lerp speed when moving the character to the climb position

        private static readonly float PIXEL_SIZE = 0.03125f; // size of one pixel: 1/32

        private static readonly float
            SURFACE_ANGLE_LIMIT =
                46.0f; // the max angle of a slope the character can stand on, if the slope angle is larger than this, the character will slide down the slope
        ...
        [Space] [FoldoutGroup("Movement")] public float walkSpeedMax = 2.5f; // max walk speed
        [FoldoutGroup("Movement")] public float walkAcc = 10.0f; // walking acceleration
        [Space] [FoldoutGroup("Movement")] public float runSpeedMax = 5.0f; // max run speed
        [FoldoutGroup("Movement")] public float runAcc = 15.0f; // running acceleration
        [Space] [FoldoutGroup("Movement")] public float crouchSpeedMax = 1.0f; // max move speed while crouching
        [FoldoutGroup("Movement")] public float crouchAcc = 8.0f; // crouching acceleration
        [Space] [FoldoutGroup("Movement")] public float crawlSpeedMax = 1.0f; // max move speed while crawling
        [FoldoutGroup("Movement")] public float crawlAcc = 8.0f; // crawling acceleration
        [Space] [FoldoutGroup("Movement")] public float airSpeedMax = 2.0f; // max move speed while in air
        [FoldoutGroup("Movement")] public float airAcc = 8.0f; // air acceleration

        [Space] [FoldoutGroup("Movement")]
        public float groundDrag = 10.0f; // braking acceleration (from movement to still) while on ground

        [FoldoutGroup("Movement")]
        public float airDrag = 0.75f; // braking acceleration (from movement to still) while in air

        [Space] [FoldoutGroup("Movement")] public bool jumpEnabled = true;

        [FoldoutGroup("Movement"), ShowIf("jumpEnabled")]
        public float jumpSpeed = 5.0f; // speed applied to character when jump

        [FoldoutGroup("Movement"), ShowIf("jumpEnabled")]
        public float jumpCooldown = 0.2f; // time to be able to jump again after landing

        [FoldoutGroup("Movement"), ShowIf("jumpEnabled")]
        public float
            jumpTolerance = 0.15f; // when the character's air time is less than this value, it is still able to jump

        [FoldoutGroup("Movement"), ShowIf("jumpEnabled")]
        public float
            jumpGravityMutiplier =
                0.6f; // gravity multiplier when character is jumping, should be within [0.0,1.0], set it to lower value so that the longer you press the jump button, the higher the character can jump    

        [FoldoutGroup("Movement"), ShowIf("jumpEnabled")]
        public float
            fallGravityMutiplier =
                1.3f; // gravity multiplier when character is falling, should be equal or greater than 1.0

        [Space] [FoldoutGroup("Movement")] public bool dashEnabled = true;

        [FoldoutGroup("Movement"), ShowIf("dashEnabled")]
        public float dashSpeedStart = 2.5f;

        [FoldoutGroup("Movement"), ShowIf("dashEnabled")]
        public float dashSpeedMax = 7.0f;

        [FoldoutGroup("Movement"), ShowIf("dashEnabled")]
        public float dashAcc = 20.0f;
        ...
        [Space] [FoldoutGroup("Movement")] public bool dodgeEnabled = true;

        [FoldoutGroup("Movement"), ShowIf("dodgeEnabled")]
        public float dodgeSpeedMul = 1.25f;

        [FoldoutGroup("Movement"), ShowIf("dodgeEnabled")]
        public float dodgeCooldown = 0.1f;

        [Space] [FoldoutGroup("Movement")] public bool ladderClimbEnabled = true;

        [FoldoutGroup("Movement"), ShowIf("ladderClimbEnabled")]
        public float ladderClimbSpeed = 1.0f; // laddder climb speed

        [FoldoutGroup("Movement"), ShowIf("ladderClimbEnabled")]
        public float ladderClimbSpeedFast = 1.5f; // laddder climb speed when move modifier key is pressed

        [Space] [FoldoutGroup("Movement")] public bool ledgeClimbEnabled = true;
        ...
        [Space] [FoldoutGroup("Attack")] public float throwForce = 10.0f;
        [FoldoutGroup("Attack")] public float throwAngularSpeed = 200.0f;
        [Space] [FoldoutGroup("Attack")] public float projectileSpeed = 20.0f;
        ...
        [Space] [FoldoutGroup("Event")] public UnityEvent onDodgeStart;
        [FoldoutGroup("Event")] public UnityEvent onDodgeEnd;
        [Space] [FoldoutGroup("Event")] public UnityEvent onAttackStart;
        ...
        [FoldoutGroup("Input"), DisableInEditMode]
        public Vector2
            inputMove = Vector2
                .zero; // movement input, x for horizontal, y for vertical, x and y should be in [-1.0, 1.0]                 

        [FoldoutGroup("Input"), DisableInEditMode]
        public bool inputRun = false; // run input

        [FoldoutGroup("Input"), DisableInEditMode]
        public bool inputDash = false; // dash input

        [FoldoutGroup("Input"), DisableInEditMode]
        public bool inputDodge = false; // dodge input

        [FoldoutGroup("Input"), DisableInEditMode]
        public bool inputCrounch = false; // crourch input

        [FoldoutGroup("Input"), DisableInEditMode]
        public bool inputCrawl = false; // crawl input
        ...
        [FoldoutGroup("Input"), DisableInEditMode]
        public bool inputMelee = false; // melee input

        [FoldoutGroup("Input"), DisableInEditMode]
        public bool inputLook = false; // look input

        [FoldoutGroup("Input"), DisableInEditMode]
        public Vector2 inputTarget = Vector2.zero; // the look at and point at target
...
#if UNITY_EDITOR
        [FoldoutGroup("Debug"), LabelText("Draw Ledge Position")]
        public bool debug_drawLedgePos;

        [FoldoutGroup("Debug"), LabelText("Draw Ledge Raycast")]
        public bool debug_drawLedgeRaycast;

        [FoldoutGroup("Debug"), LabelText("Draw Ground Check Raycast")]
        public bool debug_drawGroundCheckRaycast;

        [FoldoutGroup("Debug"), LabelText("Draw Ground Normal")]
        public bool debug_drawGroundNormal;

        [FoldoutGroup("Debug"), LabelText("Draw Ground Direction")]
        public bool debug_drawGroundDir;

        [FoldoutGroup("Debug"), LabelText("Draw Crouch Raycast")]
        public bool debug_drawCrouchRaycast;

        [FoldoutGroup("Debug"), LabelText("Draw Crawl Enter Raycast")]
        public bool debug_drawCrawlEnterRaycast;

        [FoldoutGroup("Debug"), LabelText("Draw Crawl Exit Raycast")]
        public bool debug_drawCrawlExitRaycast;

        [FoldoutGroup("Debug"), LabelText("Draw Velocity")]
        public bool debug_drawVelocity;

        [FoldoutGroup("Debug"), LabelText("Draw Slide Velocity")]
        ...
        public bool IsIdle {
            get {
                return (isMoving == false && IsInAir == false && isClimbingLadder == false &&
                        isExitingLadder == false && isClimbingLedge == false);
            }
        }
        ...
        [FoldoutGroup("Runtime"), ShowInInspector, ReadOnly]
        public bool IsInAir {
            get { return (!isGrounded && !isClimbingLadder && !isExitingLadder && !isClimbingLedge); }
        }
        ...
        public bool IsStandingOnPlatform {
            get { return isStandingOnPlatform; }
        }
        ...
        private Vector3 CenterPos {
            get { return transform.position + Vector3.up * CENTER_POS_OFFSET; }
        }

        //world position of the character's neck
        private Vector3 NeckPos {
            get { return transform.position + Vector3.up * NECK_POS_OFFSET; }
        }

        //world position of the character collider's top
        private Vector3 TopPos {
            get { return transform.position + TOP_POS_OFFSET; }
        }

        //world position of the character collider's bottom
        private Vector3 BottomPos {
            get { return transform.position + BOTTOM_POS_OFFSET; }
        }
        ...
        private void Attack() {
            ...
            if (attackAction == AttackActionType.Archery) {
                if (isCrawling == false && character.Weapon &&
                    character.Weapon.TryGetComponent<WeaponBow>(out weaponBow)) {
            ...
            else if (attackAction == AttackActionType.PointAtTarget || attackAction == AttackActionType.Summon ||
                     attackAction == AttackActionType.Throw) {
        ...
        private float CrouchRaycastDis {
            get { return COLLIDER_SIZE.y + PIXEL_SIZE; }
        }
        ...
        private Vector2 CrawlExitRaycastPos {
            get { return new Vector2(transform.position.x, transform.position.y + 10 * PIXEL_SIZE); }
        }

        private float CrawlExitRaycastDis {
            get { return 37 * PIXEL_SIZE; }
        }

        private Vector2 CrawlEnterRaycastPos {
            get { return new Vector2(transform.position.x, transform.position.y + 16 * PIXEL_SIZE); }
        }

        private float CrawlEnterRaycastDis {
            get { return 32 * PIXEL_SIZE; }
        }
        ...
        private bool EnableSpineRot {
            get { return !animator.GetBool("IsCrawling"); }
        }
        ...
        private void LookAtTarget() {
            ...
            lookAtTargetPercent = Mathf.Lerp(lookAtTargetPercent, isLookingAtTarget ? 1.0f : 0.0f,
                LOOK_AT_TARGET_LERP_SPEED * Time.deltaTime);
        ...
        public Vector2 PointAtTargetDirection {
            get { return (inputTarget - (Vector2) character.rigUpperArmR.position).normalized; }
        }
        ...
        private void PointAtTarget() {
            ...
            pointAtTargetPercent = Mathf.Lerp(pointAtTargetPercent, isPointingAtTarget ? 1.0f : 0.0f,
                POINT_AT_TARGET_LERP_SPEED * Time.deltaTime);
            ...
            targetArmRot = targetArmRot + spine1Rot + spine2Rot +
                           character.rigPelvis.transform.localRotation.eulerAngles.z;
        ...
        private static readonly float
            MOVE_THRESHOLD =
                0.05f; // when move input value is bigger than this then it is considered there is a movement input
        ...
        private float
            moveBlend; // current move blend, for blending idle, walk, run animation, lerps to target move blend on frame update
        ...
        private void Move() {
            ...
            if (isGrounded) {
                ...
                if (targetSurfaceSpeedMul < 1.0f)
                    surfaceSpeedMul = Mathf.Lerp(surfaceSpeedMul, targetSurfaceSpeedMul, 1.0f * Time.fixedDeltaTime);
            ...
            if (isGrounded) {
                ...
                if (Mathf.Abs(inputMove.x) > MOVE_THRESHOLD && speed > max)
                    speed = Mathf.MoveTowards(speed, max, dragAcc * Time.fixedDeltaTime);
                if (Mathf.Abs(inputMove.x) <= MOVE_THRESHOLD)
                    speed = Mathf.MoveTowards(speed, 0.0f, dragAcc * Time.fixedDeltaTime);
        ...
        private void JumpUpdate() {
            ...
            if (startJumpVel.magnitude > 0.01f) {
                ...
                for (int i = 0; i < standingColliders.Count; i++) {
                    if (standingColliders[i].attachedRigidbody)
                        standingColliders[i].attachedRigidbody.AddForceAtPosition(force, standingPosList[i]);
                }
        ...
        private float
            ladderExitHeight; //y distance from the button of the character to the surface when exiting from ladder

        private float ladderToAirTimer;

        private float
            ladderExitPosZ; // origin position z of the character, used for resetting the position z after exiting the ladder
        ...
        private void LadderEnterCheck() {
            ...
            //climb up to ladder
            if (inputMove.y > MOVE_THRESHOLD && NeckPos.y < ladder.TopPos.y && CenterPos.y > ladder.BottomPos.y)
        ...
        private void LedgeClimbCheck() {
            if (isDead || ledgeClimbEnabled == false || isClimbingLadder || isExitingLadder || isDrawingBow ||
                isCrawling || isDodging) {
                isClimbingLedge = false;
                return;
            }
        ...
        private void LedgeClimb() {
            if (isClimbingLedge) {
                ...
                RaycastHit2D hit = Raycast(ledgePos + new Vector2(0.0f, PIXEL_SIZE), Vector2.down, PIXEL_SIZE * 4.0f,
                    true);
                if (hit.collider) {
        ...
        private Vector3 GroundRaycastPosF {
            get {
                return GroundRaycastPosM +
                       new Vector3((isCrawling ? 15 * PIXEL_SIZE : 6 * PIXEL_SIZE) * facing, 0.0f, 0.0f);
            }
        }
        ...
        private static readonly float
            SLIDE_SPEED_MAX = 5.0f; // the max speed the character slide down a slope it can not stand on
        ...
        private void OnDrawGizmos() {
            ...
            if (debug_drawLedgeRaycast) {
                ...
                Gizmos.DrawLine(LedgeClimbRaycastPos,
                    LedgeClimbRaycastPos - new Vector3(0.0f, LedgeClimbRaycastDis, 0.0f));
            }
            ...
            if (debug_drawGroundNormal && isGrounded) {
                ...
                Gizmos.DrawLine(transform.position - Vector3.forward,
                    transform.position + new Vector3(surfaceNormal.x, surfaceNormal.y) * 2.0f - Vector3.forward);
            }
            ...
            if (debug_drawGroundDir && isGrounded) {
                Gizmos.color = Color.green;
                Gizmos.DrawLine(transform.position - Vector3.forward,
                    transform.position + new Vector3(surfaceDir.x, surfaceDir.y) * 2.0f - Vector3.forward);
            }
            ...
            if (debug_drawVelocity) {
                Gizmos.color = Color.yellow;
                Gizmos.DrawLine(transform.position - Vector3.forward,
                    transform.position - Vector3.forward + new Vector3(velocity.x, velocity.y, 0.0f));
            }
            ...
            if (debug_drawSlideVelocity && isSliding) {
                Gizmos.color = Color.yellow;
                Gizmos.DrawLine(transform.position - Vector3.forward,
                    transform.position - Vector3.forward + new Vector3(slideVel.x, slideVel.y, 0.0f));
            }
        }
#endif
        ...
        private RaycastHit2D Raycast(Vector2 origin, Vector2 direction, float distance, bool ignorePlatform = false,
            bool skipIgnoredPlatforms = true) {
