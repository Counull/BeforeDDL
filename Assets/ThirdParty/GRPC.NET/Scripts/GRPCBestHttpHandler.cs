...
namespace GRPC.NET {
    public class GRPCBestHttpHandler : HttpClientHandler {
         ...
         */
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage grpcRequest,
            ...
            bestRequest.UploadSettings.OnHeadersSent += _ => {
                grpcRequest.Content.ReadAsStreamAsync()
                    .ContinueWith(_ => { outgoingDataStream.Close(); }, cancellationToken);
            };
            ...
            bool isHeader = true;
            bestRequest.DownloadSettings.OnHeadersReceived += (HTTPRequest _, HTTPResponse response,
            ...
            bestRequest.Callback += (request, response) => {
                ...
                if (request.State != HTTPRequestStates.Finished) {
                    var ex = request.Exception ??
                             new Exception($"Unknown error while processing grpc req/resp (state={request.State}).");
