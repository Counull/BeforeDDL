...
namespace HutongGames.PlayMaker.Actions {
    ...
    public class GetTouchInfo : FsmStateAction {
        ...
        [UIHint(UIHint.Variable)] [Tooltip("Store the position of the touch in a Vector3 Variable.")]
        public FsmVector3 storePosition;

        [UIHint(UIHint.Variable)] [Tooltip("Store the X position 爄n a Float Variable.")]
        public FsmFloat storeX;

        [UIHint(UIHint.Variable)] [Tooltip("Store the Y position 爄n a Float Variable.")]
        public FsmFloat storeY;

        [UIHint(UIHint.Variable)] [Tooltip("Store the movement of the touch in a Vector3 Variable.")]
        public FsmVector3 storeDeltaPosition;

        [UIHint(UIHint.Variable)] [Tooltip("Store the X movement in a Float Variable.")]
        public FsmFloat storeDeltaX;

        [UIHint(UIHint.Variable)] [Tooltip("Store the Y movement in a Float Variable.")]
        public FsmFloat storeDeltaY;

        [UIHint(UIHint.Variable)] [Tooltip("Store the time between touch events in a Float Variable.")]
        public FsmFloat storeDeltaTime;

        [UIHint(UIHint.Variable)] [Tooltip("Store the number of tap count of the touch (e.g. 2 = double tap).")]
        public FsmInt storeTapCount;

        [Tooltip("Repeat every frame.")] public bool everyFrame = true;
            ...
            if (touchCount > 0)
            {
                foreach (var touch in Touchscreen.current.touches)
                {
                    if (fingerId.IsNone || touch.touchId.ReadValue() == fingerId.Value)
                    {
                        ...
                        float deltaX =
 normalize.Value == false ? touchDeltaPosition.x : touchDeltaPosition.x / screenWidth;
                        float deltaY =
 normalize.Value == false ? touchDeltaPosition.y : touchDeltaPosition.y / screenHeight;
