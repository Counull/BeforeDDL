    ...
    public static class ActionHelpers {
        ...
        public static Vector3 GetPosition(FsmGameObject fsmGameObject, FsmVector3 fsmVector3) {
            ...
            if (fsmGameObject.Value != null) {
                finalPos = !fsmVector3.IsNone
                    ? fsmGameObject.Value.transform.TransformPoint(fsmVector3.Value)
                    : fsmGameObject.Value.transform.position;
            }
            else {
        ...
        /// </summary>
        public static string AutoName(FsmStateAction action, Fsm fsm, FsmOwnerDefault target,
            params INamedVariable[] exposedFields) {
        ...
        /// ActionName: ownerDefault field1 field2 ...
        /// </summary>
        public static string AutoName(string actionName, Fsm fsm, FsmOwnerDefault target,
            params INamedVariable[] exposedFields) {
        ...
        /// </summary>
        public static string AutoNameConvert(FsmStateAction action, NamedVariable fromVariable,
        ...
        public static string AutoNameConvert(string actionName, NamedVariable fromVariable, NamedVariable toVariable) {
            return actionName.Replace("Convert", "") + colon + GetValueLabel(fromVariable) + " to " +
                   GetValueLabel(toVariable);
        }
        ...
        public static void DrawSpoke(Vector2 center, float angle, float minLength, float maxLength, float xScale = 1f,
            float yScale = 1f) {
            Vector2 start, end;
        ...
        public static void DrawArrow(Vector3 fromPos, Vector3 toPos, Color color, float arrowScale = 0.2f) {
            ...
            Handles.ConeHandleCap(0, fromPos + direction.normalized * (distance - arrowSize), lookAtRotation, arrowSize,
                EventType.Repaint); // fudge factor to position cap correctly
#else
        ...
        public static bool IsHotControl(bool hotControlFlag) {
            ...
            if ((tempString == "xAxis" || tempString == "yAxis" || tempString == "zAxis") &&
                GUIUtility.hotControl != 0) {
                hotControlFlag = true; // declared elsewhere
                //Debug.Log ("dragging");
            }

            if ((tempString == "xAxis" || tempString == "yAxis" || tempString == "zAxis") && hotControlFlag &&
                GUIUtility.hotControl == 0) {
        ...
        public static void RotateHandlesAroundPivot(float angle, Vector3 pivotPoint) {
            ...
            Handles.matrix = Matrix4x4.TRS((Vector3) vector3, Quaternion.Euler(0.0f, 0.0f, angle), Vector3.one) *
                             Matrix4x4.TRS((Vector3) (-vector3), Quaternion.identity, Vector3.one) * matrix;
        }
        ...
        /// </summary>
        public static void DrawWireBounds(Transform transform, Vector3 translate, Quaternion rotate, Vector3 scale,
            Color color) {
            var matrix = Handles.matrix;
            Handles.matrix = transform.localToWorldMatrix;
        ...
        public static void DrawWireCube(Vector3 position, Vector3 size, Color color) {
            ...
            var half = size / 2;
            // draw front
            Handles.DrawLine(position + new Vector3(-half.x, -half.y, half.z),
