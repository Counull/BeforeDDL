...
namespace HutongGames.PlayMaker.Actions {
    ...
    [Tooltip(
        "Move a GameObject to another GameObject using easing functions. Works like iTween Move To, but with better performance.")]
    public class MoveObject : EaseFsmAction {
        [RequiredField] [Tooltip("The GameObject to move.")]
        public FsmOwnerDefault objectToMove;

        [RequiredField] [Tooltip("The target GamObject.")]
        ...
        public override void OnUpdate() {
            ...
            if (finishAction && !finishInNextStep) {
                go.transform.position = new Vector3(
                    reverse.IsNone ? toVector.Value.x : reverse.Value ? fromValue.Value.x : toVector.Value.x,
