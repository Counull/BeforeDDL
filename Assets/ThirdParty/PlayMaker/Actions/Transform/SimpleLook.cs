...
namespace HutongGames.PlayMaker.Actions {
    ...
    public class SimpleLook : ComponentAction<Transform> {
        ...
        [RequiredField] [Tooltip("Vector2 input, typically from a PlayerInput action.")]
        public FsmVector2 vector2Input;

        [RequiredField] [Tooltip("Sensitivity of movement in X direction (rotate left/right).")]
        public FsmFloat sensitivityX;

        [RequiredField] [Tooltip("Sensitivity of movement in Y direction (tilt up/down).")]
        public FsmFloat sensitivityY;

        [HasFloatSlider(-360, 360)] [Tooltip("Clamp rotation around X axis. Set to None for no clamping.")]
        public FsmFloat minimumX;

        [HasFloatSlider(-360, 360)] [Tooltip("Clamp rotation around X axis. Set to None for no clamping.")]
        public FsmFloat maximumX;

        [HasFloatSlider(-360, 360)] [Tooltip("Clamp rotation around Y axis. Set to None for no clamping.")]
        ...
        private void DoLookRotate() {
            ...
            if (cachedCameraTransform == null) {
                ...
                cachedCameraTransform.localEulerAngles =
                    new Vector3(GetYRotation(-1), cachedCameraTransform.localEulerAngles.y, 0);
            }
        }
