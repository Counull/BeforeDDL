...
namespace HutongGames.PlayMaker.Actions {
    ...
    public class LoadSceneAsynch : FsmStateAction {
        ...
        [Tooltip(
            "Use this hash to activate the Scene if you have set 'AllowSceneActivation' to false, you'll need to use it in the action 'AllowSceneActivation' to effectively load the scene.")]
        [UIHint(UIHint.Variable)]
        public FsmInt aSyncOperationHashCode;

        [Tooltip("The loading's progress.")] [UIHint(UIHint.Variable)]
        public FsmFloat progress;

        [Tooltip("True when loading is done")] [UIHint(UIHint.Variable)]
        public FsmBool isDone;

        [Tooltip("True when loading is done but still waiting for scene activation")] [UIHint(UIHint.Variable)]
        ...
        public override void OnUpdate() {
            ...
            if (_asyncOperation.isDone) {
                ...
                if (_asyncOperation.progress == 0.9f && _asyncOperation.allowSceneActivation == false &&
                    !pendingActivationCallBackDone) {
