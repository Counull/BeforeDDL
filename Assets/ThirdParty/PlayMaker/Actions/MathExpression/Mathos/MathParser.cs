...
namespace Mathos.Parser {
    ...
    public class MathParser {
        ...
        /// <returns>The result of executing <paramref name="mathExpression"/>.</returns>
        public double ProgrammaticallyParse(string mathExpression, bool correctExpression = true,
            bool identifyComments = true) {
            if (identifyComments) {
            ...
            while (tokens.IndexOf("(") != -1) {
                ...
                if (open >= close)
                    throw new ArithmeticException(
                        "No closing bracket/parenthesis. Token: " + open.ToString(CultureInfo));
                ...
                if (LocalFunctions.Keys.Contains(functionName)) {
                    if (roughExpr.Contains(",")) {
                        ...
                        tmpResult = double.Parse(LocalFunctions[functionName](args.ToArray()).ToString(CultureInfo),
                            CultureInfo);
                    }
                    else {
        ...
        private double BasicArithmeticalExpression(List<string> tokens) {
            ...
            switch (tokens.Count) {
                ...
                case 2:
                    ...
                    if (op == "-" || op == "+") {
                        return double.Parse(
                            (op == "+" ? "" : (tokens[1].Substring(0, 1) == "-" ? "" : "-")) + tokens[1], CultureInfo);
                    }
