...
namespace HutongGames.PlayMaker.Actions {
    ...
    public class EaseColor : EaseFsmAction {
        [RequiredField] [Tooltip("The Color value to ease from.")]
        public FsmColor fromValue;

        [RequiredField] [Tooltip("The Color value to ease to.")]
        public FsmColor toValue;

        [UIHint(UIHint.Variable)] [Tooltip("Store the result in a Color Variable.")]
        ...
        public override void OnUpdate() {
            ...
            if (finishAction && !finishInNextStep) {
                if (!colorVariable.IsNone) {
                    colorVariable.Value = new Color(
                        reverse.IsNone ? toValue.Value.r : reverse.Value ? fromValue.Value.r : toValue.Value.r,
