...
namespace HutongGames.PlayMaker.Actions {
    ...
    public class EaseFloat : EaseFsmAction {
        [RequiredField] [Tooltip("The float value to ease from.")]
        public FsmFloat fromValue;

        [RequiredField] [Tooltip("The float value to ease to.")]
        public FsmFloat toValue;

        [UIHint(UIHint.Variable)] [Tooltip("Store the result in a Float Variable.")]
        ...
        public override void OnUpdate() {
            ...
            if (finishAction && !finishInNextStep) {
                if (!floatVariable.IsNone) {
                    floatVariable.Value =
                        reverse.IsNone ? toValue.Value : reverse.Value ? fromValue.Value : toValue.Value;
                }
