...
namespace HutongGames.PlayMaker.Actions {
    ...
    public class EaseRect : EaseFsmAction {
        [RequiredField] [Tooltip("Ease from this Rect value.")]
        public FsmRect fromValue;

        [RequiredField] [Tooltip("Ease to this Rect value.")]
        public FsmRect toValue;

        [UIHint(UIHint.Variable)] [Tooltip("Store the current value in a Rect Variable.")]
        ...
        public override void OnUpdate() {
            ...
            if (finishAction && !finishInNextStep) {
                if (!rectVariable.IsNone) {
                    rectVariable.Value = new Rect(
                        reverse.IsNone ? toValue.Value.x : reverse.Value ? fromValue.Value.x : toValue.Value.x,
                        reverse.IsNone ? toValue.Value.y : reverse.Value ? fromValue.Value.y : toValue.Value.y,
                        reverse.IsNone ? toValue.Value.width :
                        reverse.Value ? fromValue.Value.width : toValue.Value.width,
                        reverse.IsNone ? toValue.Value.height :
                        reverse.Value ? fromValue.Value.height : toValue.Value.height
                    );
                }
