...
namespace HutongGames.PlayMakerEditor {
    ...
    public class FindOverlapsEditor : CustomActionEditor {
        ...
        public override void OnSceneGUI() {
            ...
            var newCenter = Handles.PositionHandle(action.center,
                transform != null ? transform.rotation : Quaternion.identity);

            Handles.matrix = Matrix4x4.TRS(action.center, transform != null ? transform.rotation : Quaternion.identity,
                Vector3.one);
            ...
            if (EditorGUI.EndChangeCheck()) {
                action.offset.Value = transform == null ? newCenter : transform.InverseTransformPoint(newCenter);
