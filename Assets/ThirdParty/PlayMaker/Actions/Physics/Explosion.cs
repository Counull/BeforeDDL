...
namespace HutongGames.PlayMaker.Actions {
    ...
    public class Explosion : FsmStateAction {
        ...
        [RequiredField] [Tooltip("The radius of the explosion. Force falls of linearly with distance.")]
        public FsmFloat radius;

        [Tooltip(
            "Applies the force as if it was applied from beneath the object. This is useful because explosions that throw things up instead of pushing things to the side look cooler. A value of 2 will apply a force as if it is applied from 2 meters below while not changing the actual explosion position.")]
        ...
        //Removed
        [System.NonSerialized] [UIHint(UIHint.Layer)]
        public FsmInt layer;
        ...
        void DoExplosion() {
            ...
            foreach (var hit in colliders) {
                ...
                if (rigidBody != null && ShouldApplyForce(hit.gameObject)) {
                    rigidBody.AddExplosionForce(force.Value, center.Value, radius.Value, upwardsModifier.Value,
                        forceMode);
                }
            }
        }
