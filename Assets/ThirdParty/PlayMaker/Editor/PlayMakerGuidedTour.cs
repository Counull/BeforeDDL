...
namespace HutongGames.PlayMakerEditor {
    ...
    public class PlayMakerGuidedTour : BaseGuidedTourWindow {
        ...
        private void BuildMainEditorGuide() {
            ...
            var selectionToolbar = AddTopic(root, "Selection Toolbar",
                "The Selection Toolbar lets you quickly select FSMs in the scene.", "", "",
                GetUrl(WikiPages.SelectionToolbar));
            ...
            AddTopic(selectionToolbar, "Selected FSM", "Select an FSM on the GameObject.",
                "NOTE: A GameObject can have multiple FSMs.");
            ...
            var debugToolbar = AddTopic(root, "Debug Toolbar",
                "The Debug Toolbar collects tools for debugging FSMs in the editor.", "", "",
                GetUrl(WikiPages.DebuggingToolbar));
        ...
        private void BuildActionBrowserGuide() {
            var root = AddWindow(typeof(FsmActionWindow), "Action Browser", "Browse for actions to add to a state.",
                ...
                "check out <a href=\"W174\">Core Concepts > Actions</a> in the online manual.", "Window",
                GetUrl(WikiPages.ActionBrowser));
            ...
            AddTopic(root, "Add Action To State", "Add selected action to the selected state.",
                "NOTE: You can also double click the action; Or drag and drop actions from the Action Browser into the State Inspector or Graph View.",
                "Add Action");
        }
        ...
        private void BuildTemplatesBrowserGuide() {
            var root = AddWindow(typeof(FsmTemplateWindow), "Templates Browser", "Shows all saved FSM Templates.",
                ...
                "This window lets you manage the Templates in your project.", "Window",
                GetUrl(WikiPages.TemplatesBrowser));
            AddTopic(root, "Settings Menu", "Settings for this window.");
            AddTopic(root, "Search", "Interactively search as you type.");
            AddTopic(root, "Search Mode", "Controls how to perform the search.",
        ...
        private void BuildFsmTimelineGuide() {
            var root = AddWindow(typeof(FsmTimelineWindow), "Timeline Log", "A visual log of FSM state changes.",
                "NOTE: This is not connected to the new Unity Timeline feature!", "Window",
                GetUrl(WikiPages.FsmTimeline));
        ...
        private void BuildFsmLogGuide() {
            var root = AddWindow(typeof(FsmLogWindow), "FSM Log", "A log of all FSM Events",
                ...
                "HINT: Click <b>SentBy</b> to jump to the GameObject/FSM that sent an event.", "Window",
                GetUrl(WikiPages.FsmLog));
            AddTopic(root, "Settings Menu", "Settings for this window.",
