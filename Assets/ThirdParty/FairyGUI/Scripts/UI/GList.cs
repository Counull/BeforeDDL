...
namespace FairyGUI {
    ...
    public class GList : GComponent {
            ...
            if (layoutChanged) {
                if (_layout == ListLayoutType.SingleColumn || _layout == ListLayoutType.SingleRow)
                    ...
                    if (_columnCount > 0)
                        ...
                        _curLineItemCount =
                            Mathf.FloorToInt((this.scrollPane.viewWidth + _columnGap) / (_itemSize.x + _columnGap));
                    ...
                    if (_lineCount > 0)
                        ...
                        _curLineItemCount =
                            Mathf.FloorToInt((this.scrollPane.viewHeight + _lineGap) / (_itemSize.y + _lineGap));
                        if (_curLineItemCount <= 0)
                            _curLineItemCount = 1;
                    }
                }
        ...
        bool HandleScroll1(bool forceUpdate) {
            ...
            if (curIndex > 0 && this.numChildren > 0 && this.container.y <= 0 &&
                GetChildAt(0).y > -this.container.y) //最后一页没填满！
                return true;
            else
                return false;
        }
        ...
        bool HandleScroll2(bool forceUpdate) {
            ...
            if (curIndex > 0 && this.numChildren > 0 && this.container.x <= 0 &&
                GetChildAt(0).x > -this.container.x) //最后一页没填满！
                return true;
            else
                return false;
        }
