...
namespace FairyGUI {
    ...
    public class ScrollPane : EventDispatcher {
        ...
        public float scrollingPosX {
            get { return Mathf.Clamp(-_container.x, 0, _overlapSize.x); }
        }
        ...
        public float scrollingPosY {
            get { return Mathf.Clamp(-_container.y, 0, _overlapSize.y); }
        }
        ...
        void HandleSizeChanged() {
            if (_displayInDemand) {
                ...
                if (_vtScrollBar != null && _hzScrollBar != null) {
                    if (!_hScrollNone)
                        _vtScrollBar.height = _owner.height - _hzScrollBar.height - _scrollBarMargin.top -
                                              _scrollBarMargin.bottom;
                    else
                        _vtScrollBar.height = _owner.height - _scrollBarMargin.top - _scrollBarMargin.bottom;

                    if (!_vScrollNone)
                        _hzScrollBar.width = _owner.width - _vtScrollBar.width - _scrollBarMargin.left -
                                             _scrollBarMargin.right;
                    else
