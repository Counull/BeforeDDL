        ...
        public override void OnInspectorGUI() {
            ...
            if (gObj != null) {
                ...
                if (gObj.parent != null) {
                    ...
                    EditorGUI.BeginChangeCheck();
                    int childIndex = EditorGUILayout.IntPopup("Child Index", gObj.parent.GetChildIndex(gObj), options,
                        values);
                    if (EditorGUI.EndChangeCheck())
                ...
                if (textFormat != null) {
                    _guiTextFormatFoldout =
                        EditorGUILayout.BeginFoldoutHeaderGroup(_guiTextFormatFoldout, "Text Format");
                    EditorGUI.BeginChangeCheck();
                    if (_guiTextFormatFoldout) {
                        ...
                        textFormat.shadowOffset =
                            EditorGUILayout.Vector2Field("Shadow Offset", textFormat.shadowOffset);
                        ...
                        EditorGUILayout.BeginHorizontal();
                        textFormat.italic = EditorGUILayout.ToggleLeft("<i>I</i>", textFormat.italic, richStyle,
                            GUILayout.Width(30));
                        textFormat.bold = EditorGUILayout.ToggleLeft("<b>B</b>", textFormat.bold, richStyle,
                            GUILayout.Width(30));
                        textFormat.underline = EditorGUILayout.ToggleLeft("U̲", textFormat.underline, richStyle,
                            GUILayout.Width(30));
                        textFormat.strikethrough = EditorGUILayout.ToggleLeft(" S̶ ̶ ̶", textFormat.strikethrough,
                            richStyle, GUILayout.Width(36));
                        EditorGUILayout.EndHorizontal();
                        ...
                        EditorGUIUtility.labelWidth = initLabelWidth;
                        EditorGUILayout.EndHorizontal();
                        textFormat.specialStyle =
                            (TextFormat.SpecialStyle) EditorGUILayout.EnumPopup("Special Style",
                                textFormat.specialStyle);
                    }
                ...
                if (gObj is GComponent gComp) {
                    ...
                    if (gComp.Controllers.Count > 0) {
                        _guiControllersFoldout =
                            EditorGUILayout.BeginFoldoutHeaderGroup(_guiControllersFoldout, "Controllers");
                        if (_guiControllersFoldout) {
                            foreach (var ctl in gComp.Controllers) {
                                EditorGUILayout.BeginHorizontal();
                                EditorGUILayout.LabelField(ctl.name, headerLabelStyle,
                                    GUILayout.MaxWidth(ctl.name.Length * 15));
                    ...
                    if (gComp.Transitions.Count > 0) {
                        _guiTransitionsFoldout =
                            EditorGUILayout.BeginFoldoutHeaderGroup(_guiTransitionsFoldout, "Transitions");
                        if (_guiTransitionsFoldout) {
                            foreach (var transition in gComp.Transitions) {
                                ...
                                EditorGUILayout.LabelField($"{transition.name} - {transition.totalDuration}s",
                                    labelStyle, GUILayout.MinWidth(150));
                                EditorGUI.BeginChangeCheck();
